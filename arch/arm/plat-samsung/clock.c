...
/**
 * s3c_enable_clocks() - enable an array of clocks
 * @clkp: Pointer to the first clock in the array.
 * @nr_clks: Number of clocks to register.
 *
...
void __init s3c_enable_clocks(struct clk *clkp, int nr_clks)
{
	for (; nr_clks > 0; nr_clks--, clkp++)
		(clkp->enable)(clkp, 1);
}
...
static void clock_tree_show_one(struct seq_file *s, struct clk *c, int level)
{
	...
	list_for_each_entry(child, &clocks, list) {
		...
		/* audss is powered off at run-time if no audio output, so, can
		 * not be accessed, don't access it. */
		if (strcmp(child->name, "dout_srp") == 0)
			continue;
