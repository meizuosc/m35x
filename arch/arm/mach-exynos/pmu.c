...
static bool xxti_sys_powerdown_mode = false;
...
static struct exynos_pmu_conf exynos5410_pmu_config[] = {
	...
	{ EXYNOS5410_PAD_RETENTION_HSI_SYS_PWR,			{ 0x1, 0x1, 0x0} },
...
#if 0
	/*xxti_sys_powerdown for codec 24M for meizu*/
	if (soc_is_exynos5410() &&
		!xxti_sys_powerdown_mode && mode == SYS_SLEEP) {
		exynos_xxti_sys_powerdown(0);
	}
#endif
}

void exynos_xxti_sys_powerdown_enable(bool mode)
{
	xxti_sys_powerdown_mode = !!mode;
}
...
static int __init exynos_pmu_init(void)
{
	...
	if (soc_is_exynos4210()) {
		...
		/* Set Stable counter */
		__raw_writel(0x8020, EXYNOS5_XXTI_DURATION3);
		__raw_writel(0x8020, EXYNOS5_EXT_REGULATOR_DURATION3);
