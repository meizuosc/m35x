...
static int exynos5_gsc_clock_init(void)
{
	...
	for (i = 0; i < MAX_CAM_NUM; i++) {
		sprintf(sensor_name, "sclk_isp_sensor%d", i);
...
static int exynos5_mmc_clock_init(void)
{
	...
	struct clk *dw_mmc0, *dw_mmc1, *dw_mmc2, *mout_epll;
	...
	if (clk_set_parent(sclk_mmc0, mout_epll))
		pr_err("Unable to set parent %s of clock %s.\n",
				mout_cpll->name, sclk_mmc0->name);
	if (clk_set_parent(sclk_mmc1, mout_epll))
		pr_err("Unable to set parent %s of clock %s.\n",
				mout_epll->name, sclk_mmc1->name);
...
static int exynos5_uart_clock_init(void)
{
	...
	uart_clk = uart3 = clk_get_sys("s5pv210-uart.3", "uclk1");
	mout_cpll = clk_get(NULL, "mout_cpll");
	...
	if (clk_set_parent(uart3, mout_cpll))
		...
		/* Set UART3's clock source freq same as Bootloader setting(64MHz) */
		clk_set_rate(uart3, 64 * MHZ);
...
void __init exynos5_m6x_clock_init(void)
{
