...
static int exynos5_fimc_is_pin_cfg_cam_isp_sensor_io_18v(struct platform_device *pdev, int channel, int flag_on)
{
	int ret = 0;
	int i = 0;
	...
	for (i = FIMC_IS_MAX_GPIO_NUM - 1; i >= 0; i--) {
		...
		if (gpio->count == 1) {
			/* GPIO_CAM_AF_EN and must turn off before cam_isp_sensor_io_1.8v */
			if (strcmp("GPIO_CAM_AF_EN", gpio->name) == 0 ||
				strcmp("cam_af_2.8v_pm", gpio->name) == 0) {
	...
	if (flag_on == 1) {
		...
		for (i = FIMC_IS_MAX_GPIO_NUM - 1; i >= 0; i--) {
			...
			if (gpio->count == 1) {
				...
				/* iT4 */
				if (strcmp("cam_isp_sensor_io_1.8v", gpio->name) == 0) {
					ret = exynos5_fimc_is_pin_cfg_cam_isp_sensor_io_18v(pdev, channel, flag_on);
					if (ret)
						pr_err("%s : exynos5_fimc_is_pin_cfg_cam_isp_sensor_io_18v failed\n", __func__);
...
int exynos5_fimc_is_cfg_clk(struct platform_device *pdev)
{
	...
	printk("==>[MMKIM] exynos5_fimc_is_cfg_clk\n");
...
int exynos5_fimc_is_clk_on(struct platform_device *pdev)
{
	...
	printk("==>[MMKIM] exynos5_fimc_is_clk_on\n");
...
int exynos5_fimc_is_clk_off(struct platform_device *pdev)
{
	...
	printk("==>[MMKIM] exynos5_fimc_is_clk_off\n");
