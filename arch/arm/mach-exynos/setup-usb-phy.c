...
static int exynos5_usb_host_is_suspend(void)
{
	return (readl(EXYNOS4_PHYPWR) & HOST_CTRL0_FORCESUSPEND) ? 1 : 0;
}
...
static int exynos5_usb_phy_host_suspend(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "phy enter suspend\n");
...
static int exynos5_usb_phy_host_resume(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "phy enter resume CTRL=0x%08x\n", readl(EXYNOS5_PHY_HOST_CTRL0));

	if (exynos5_usb_phy20_is_on()) {
		if (exynos5_usb_host_is_suspend()) {
			/* set to suspend HSIC 0 and 1 and standard of PHY1 */
			hostphy_ctrl0 = readl(EXYNOS5_PHY_HOST_CTRL0);
			hostphy_ctrl0 &= ~(HOST_CTRL0_FORCESUSPEND);
			writel(hostphy_ctrl0, EXYNOS5_PHY_HOST_CTRL0);
		...
		otgphy_sys &= ~(OTG_SYS_SIDDQ_UOTG | OTG_SYS_FORCE_SLEEP |
							OTG_SYS_FORCE_SUSPEND);
		...
		hostphy_ctrl0 |= (HOST_CTRL0_LINKSWRST |
...
static int exynos5_usb_phy20_init(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "Power on usb PHY\n");
...
static int exynos5_usb_phy20_exit(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "Power off usb PHY\n");

	if (atomic_dec_return(&host_usage) > 0) {
		dev_err(&pdev->dev, "still being used\n");
		return -EBUSY;
	}
...
static int exynos5_check_usb_op(void)
{
	...
	if (hostphy_ctrl0 & HOST_CTRL0_FORCESUSPEND &&
		...
		/* reset all PHY links */
		writel(readl(EXYNOS5_PHY_HOST_CTRL0)|HOST_CTRL0_PHYSWRST, EXYNOS5_PHY_HOST_CTRL0);
		writel(readl(EXYNOS5_PHY_HSIC_CTRL1)|HSIC_CTRL_PHYSWRST, EXYNOS5_PHY_HSIC_CTRL1);
		writel(readl(EXYNOS5_PHY_HSIC_CTRL2)|HSIC_CTRL_PHYSWRST, EXYNOS5_PHY_HSIC_CTRL2);
		writel(readl(EXYNOS5_PHY_HOST_CTRL0)|HOST_CTRL0_PHYSWRSTALL, EXYNOS5_PHY_HOST_CTRL0);
		udelay(80);
