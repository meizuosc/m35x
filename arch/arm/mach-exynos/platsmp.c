...
static int exynos_power_up_cpu(unsigned int cpu)
{
	...
	if (cluster) {
		timeout = 5000;		/* Wait for 50ms */
		do {
			udelay(10);
			timeout--;
		} while(!__raw_readl(EXYNOS_PMU_SPARE2) && timeout);
		udelay(10);

		if (timeout == 0) {
			printk(KERN_ERR "cpu%d power up failed for can not get value from EXYNOS_PMU_SPARE2\n", cpu);
			return -ETIMEDOUT;
		}
