...
static struct exynos5_sensor_gpio_info gpio_vision_sensor_m6x = {
	.cfg = {
		/* 2M AVDD_28V */
		/* 2M DVDD_18V */
		/* 2M VIS_STBY */
...
#endif
	.enable_rst = true, /* positive reset */
	.gpio_info = &gpio_vision_sensor_m6x,
...
static struct s5p_mfc_qos m6x_mfc_qos_table[] = {
	[0] = {
		.thrd_mb	= 0,
		.freq_mfc	= 111000,
		.freq_int	= 160000,
...
static struct s5p_mfc_platdata m6x_mfc_pd = {
	.ip_ver		= IP_VER_MFC_5A_1,
	.clock_rate	= 333 * MHZ,
	.min_rate	= 83250, /* in KHz */
#ifdef CONFIG_ARM_EXYNOS5410_BUS_DEVFREQ
	.num_qos_steps	= ARRAY_SIZE(m6x_mfc_qos_table),
	.qos_table	= m6x_mfc_qos_table,
#endif
};
#endif
...
static struct platform_device *m6x_media_devices[] __initdata = {
...
#ifdef CONFIG_VIDEO_EXYNOS5_FIMC_IS
static struct exynos5_sensor_gpio_info gpio_m6x = {
	.cfg = {
		/* 13M AVDD_28V */
		{
			.pin_type = PIN_GPIO,
...
void __init exynos5_m6x_media_init(void)
{
#if defined (CONFIG_CSI_D) || defined (CONFIG_S5K6B2_CSI_D)
...
#endif
#ifdef CONFIG_VIDEO_EXYNOS_MFC
	s5p_mfc_set_platdata(&m6x_mfc_pd);
	...
	platform_add_devices(m6x_media_devices,
			ARRAY_SIZE(m6x_media_devices));

	s3c_set_platdata(&exynos5410_scaler_pd, sizeof(exynos5410_scaler_pd),
			&exynos5_device_scaler0);
#ifdef CONFIG_VIDEO_EXYNOS_FIMC_LITE
	/*
	* May invoked here instead of using arch_initcall() in m6x_camera.c
	*/
	//m6x_init_camera();
#endif
...
/* Below should be enabled after power domain is available */
	...
	exynos5_fimc_is_data.gpio_info = &gpio_m6x;
