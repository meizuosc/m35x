...
static struct map_desc exynos5_iodesc[] __initdata = {
	{
		...
		.pfn		= __phys_to_pfn(EXYNOS5_PA_RTC),
		.length		= SZ_4K,
		.type		= MT_DEVICE,
	}, {
		.virtual	= (unsigned long)S5P_VA_SROMC,
					...
					const struct cpumask *dest, bool force)
{
	...
	desc = irq_to_desc(combiner_data[combiner_nr].parent_irq);
	if (!desc) {
		ret = -EINVAL;
		goto out;
	}

	ret = __irq_set_affinity_locked(irq_desc_get_irq_data(desc), &target_affinity);
	if (ret >= 0)
