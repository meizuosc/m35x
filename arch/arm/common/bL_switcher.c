...
static unsigned int switch_operation = 0x11;// 0x00==disable switching, 0x11==enable switching;
static unsigned int backup_switch_operation;// 0x00==disable switching, 0x11==enable switching;
...
void bL_switch_save(void)
{
	backup_switch_operation = switch_operation;
	switch_operation = 0x00;
}

void bL_switch_restore(void)
{
	switch_operation = backup_switch_operation;
}

DEFINE_SPINLOCK(switch_lock);
bool bl_switching = 0;
...
int bL_cluster_switch_request(unsigned int new_cluster)
{
	...
	if (unlikely(switch_operation == 0x00)) {
		pr_err("%s: bL switch disabled, exit!\n", __func__);
