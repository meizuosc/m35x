...
#ifdef INTERRUPT_LATENCY_CHECK
u64 interrupt_timestamp[4];
EXPORT_SYMBOL(interrupt_timestamp);
extern u32 gpio_gic_base;
#endif
...
asmlinkage void __exception_irq_entry gic_handle_irq(struct pt_regs *regs)
{
    ...
    interrupt_timestamp[smp_processor_id()]=local_clock();

    //GPX1(7) = VOL_UP
    new_gpio_base=gpio_gic_base;//ioremap(0x13400000+0xc20, 12);
    new_GPX1_base=new_gpio_base+0xC20;
    if(new_gpio_base!=0)
    {
        writel( (readl(new_GPX1_base)&~(0xf<<4*7))|(0x1<<4*7), new_GPX1_base); // GPX1CON
        writel( (readl(new_GPX1_base+8)&~(0x3<<2*7))|(0x3<<2*7), new_GPX1_base+8); // GPX1PUD
        writel( (readl(new_GPX1_base+4)&~(0x1<<7))|(0<<7), new_GPX1_base+4); // GPX1DAT
        //printk("%x=%x\n", new_GPX1_base, readl(new_GPX1_base+4)); //checked
    }
#endif
	do {
...
#ifdef INTERRUPT_LATENCY_CHECK
    if(new_gpio_base != 0){
        //writel( 0, gpio_base + 0xC04); // GPX0DAT
        writel( ((readl(new_GPX1_base+4)&~(0x1<<7))|(1<<7)), new_GPX1_base+4); // GPX1DAT
        //printk("%x=%x\n", new_GPX1_base, readl(new_GPX1_base+4)); //checked
        //iounmap(new_gpio_base);
    }
#endif
}
...
static void __init gic_dist_init(struct gic_chip_data *gic)
{
	 ...
	 */
	for (i = 32; i < gic_irqs; i += 4){
#if defined(CONFIG_FB_I80_COMMAND_MODE) || defined(CONFIG_FB_VIDEO_PSR)
        if (i == 76) // 1Byte, ID of GPIO_RT(GPH)=77, GPIO_LB(GPJ)=78
		    writel_relaxed(0xa000a0a0, base + GIC_DIST_PRI + i * 4 / 4);
        else
#endif
