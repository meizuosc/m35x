...
static struct extra_part_info battery_model_partition;
static struct extra_part_info param_partition;
...
static inline int is_private_info_area(struct request *rqc)
{
	return (blk_rq_pos(rqc) >= private_info_partition.start_sec
			&& blk_rq_pos(rqc) < private_info_partition.start_sec +
								private_info_partition.sec_count)
								||(blk_rq_pos(rqc) >= battery_model_partition.start_sec
								&& blk_rq_pos(rqc) < battery_model_partition.start_sec
								+battery_model_partition.sec_count)
								||(blk_rq_pos(rqc) >= param_partition.start_sec
								&& blk_rq_pos(rqc) < param_partition.start_sec
								+param_partition.sec_count);
}
static struct extra_part_info extra_partition[PART_MAX];
static inline int is_recovery_private_area(struct request *rqc)
{
	return (blk_rq_pos(rqc) >= extra_partition[PART_REC_PRIV].start_sec
			&& blk_rq_pos(rqc) < extra_partition[PART_REC_PRIV].start_sec +
			extra_partition[PART_REC_PRIV].sec_count);
}
...
static int deal_private_block_internal(int write, sector_t index, int len, void *buffer,int area)
{
	...
	struct part_info cur_part = basic_bootinfo.part[area];
//	pr_info(" %s(write = %d, index = %llu, len = %d, buffer) private_info_partion.sec_count = %llu\n",
//		__func__, write,(unsigned long long)index, len,(unsigned long long)cur_part.sec_count);
	...
	if(index >= cur_part.start_sec + cur_part.sec_count || index < cur_part.start_sec){
		pr_err("%s index invalid\n",__func__);
		return -EINVAL;
	}
...
int deal_private_block(int write, unsigned offset , long len, void *buffer, int area)
{
	int ret = 0;
	sector_t index;
	void *p = buffer;

//	pr_info("%s(write=%d, offset=%u, len = %ld)\n", __func__,write, offset, len);
	if(offset % MMC_BLOCK_SIZE)
		return -EINVAL;

	index = offset / MMC_BLOCK_SIZE;
//	pr_info("%s index = offset/MMC_BLOCK_SIZE = %u", __func__, index);
	...
	while(len > 0) {
		...
		ret = deal_private_block_internal(write, index, size, p, area);
		if(ret) {
			pr_err("%s (%ld) error %d\n", __func__, len, ret);
			break;
		}
		pr_debug("%s index %llu remain %ld pointer %p\n", __func__, (unsigned long long)index, len, p);
	...
	while (reqs < max_packed_rw - 1) {
		...
		if (rq_data_dir(next) == WRITE) {
			if(is_userspace_area(next)) {
				...
				if(!is_recovery_private_area(req)){
					put_back = 1;
					break;
				}
			}
...
static int init_extra_partition(struct mmc_blk_data *md)
{
#if defined(CONFIG_RECOVERY_KERNEL) || defined(CONFIG_EXPORT_KIMAGE_PARTITION)
	int i;
	int index;
	int k;
	if(!is_charging_mode()){
		for(k =0;k < 5; k++){
			pr_debug("%s %u %u", __func__, basic_bootinfo.part[k].start_sec,
					basic_bootinfo.part[k].sec_count);
		}
		{
			// kernel
			extra_partition[PART_KERNEL].start_sec = basic_bootinfo.part[PART_KERNEL].start_sec;
			extra_partition[PART_KERNEL].sec_count = basic_bootinfo.part[PART_KERNEL].sec_count;
			extra_partition[PART_KERNEL].volname = "KERNEL";

#ifndef CONFIG_EXPORT_KIMAGE_PARTITION
			// ramdisk
			extra_partition[PART_RAMDISK].start_sec = basic_bootinfo.part[PART_RAMDISK].start_sec;
			extra_partition[PART_RAMDISK].sec_count = basic_bootinfo.part[PART_RAMDISK].sec_count;
			extra_partition[PART_RAMDISK].volname = "RAMDISK";
			// recovery
			extra_partition[PART_RECOVERY].start_sec = basic_bootinfo.part[PART_RECOVERY].start_sec;
			extra_partition[PART_RECOVERY].sec_count = basic_bootinfo.part[PART_RECOVERY].sec_count;
			extra_partition[PART_RECOVERY].volname = "RECOVERY";
			// logo
			extra_partition[PART_LOGO].start_sec = basic_bootinfo.part[PART_LOGO].start_sec;
			extra_partition[PART_LOGO].sec_count = basic_bootinfo.part[PART_LOGO].sec_count;
			extra_partition[PART_LOGO].volname = "LOGO";
			// param
			extra_partition[PART_PARAM].start_sec = basic_bootinfo.part[PART_PARAM].start_sec;
			extra_partition[PART_PARAM].sec_count = basic_bootinfo.part[PART_PARAM].sec_count;
			extra_partition[PART_PARAM].volname = "PARAM";
			//reovery private
			extra_partition[PART_REC_PRIV].start_sec = basic_bootinfo.part[PART_REC_PRIV].start_sec;
			extra_partition[PART_REC_PRIV].sec_count = basic_bootinfo.part[PART_REC_PRIV].sec_count;
			extra_partition[PART_REC_PRIV].volname = "REC_PRIV";
#endif
		}
		...
		for (i = 0; i < PART_MAX; i++) {
			if (extra_partition[i].sec_count != 0) {
				struct partition_meta_info info;
				struct hd_struct *part;
				strncpy(info.volname, extra_partition[i].volname, sizeof(info.volname));
				pr_debug("about to add_partition %s, start_sec=%lu, sec_count=%lu\n",
						__func__, extra_partition[i].start_sec,extra_partition[i].sec_count);
				part = add_partition(md->disk, index, extra_partition[i].start_sec,
						extra_partition[i].sec_count, ADDPART_FLAG_NONE, &info);
				if (IS_ERR(part)) {
					pr_err("error add partition %s\n", extra_partition[i].volname);
				} else {
					index++;
					pr_info("add extra partition at mmcblk0p%d(%s), start_sec = %llu, sec_num = %llu\n",
							i + 5, extra_partition[i].volname,
							(unsigned long long)extra_partition[i].start_sec,
							(unsigned long long)extra_partition[i].sec_count);
				}
			}
		}
	}
	...
	extra_partition[PART_REC_PRIV].sec_count = basic_bootinfo.part[PART_REC_PRIV].sec_count;
	extra_partition[PART_REC_PRIV].volname = "REC_PRIV";
	part = add_partition(md->disk, 5+PART_REC_PRIV, extra_partition[PART_REC_PRIV].start_sec,
	extra_partition[PART_REC_PRIV].sec_count, ADDPART_FLAG_NONE, &info);
	if (IS_ERR(part)) {
		pr_err("error add partition %s\n", extra_partition[PART_REC_PRIV].volname);
	} else {
		pr_info("add extra partition at mmcblk0p%d(%s)", 5+PART_REC_PRIV, extra_partition[PART_REC_PRIV].volname);
	}
#endif

	battery_model_partition.start_sec = basic_bootinfo.part[PART_BAT_MODEL].start_sec;
	battery_model_partition.sec_count = basic_bootinfo.part[PART_BAT_MODEL].sec_count;
	battery_model_partition.volname = "BATTERY_MODEL";

	if(!is_charging_mode()){
		private_info_partition.start_sec = basic_bootinfo.part[PART_PRIVATE].start_sec;
		private_info_partition.sec_count = basic_bootinfo.part[PART_PRIVATE].sec_count;
		private_info_partition.volname = "PRIVATE";

		param_partition.start_sec = basic_bootinfo.part[PART_PARAM].start_sec;
		param_partition.sec_count = basic_bootinfo.part[PART_PARAM].sec_count;
		param_partition.volname = "PARAM";


		userspace_area.start_sec =  basic_bootinfo.part[PART_PARAM].start_sec
			+ basic_bootinfo.part[PART_PARAM].sec_count;
		userspace_area.sec_count = -1;
...
//#if 0
	...
	init_extra_partition(md);

#define CUSTOM_PARTNO	4
	if (!strcmp(md->disk->disk_name, "mmcblk0")) {
		struct disk_part_iter piter;
		struct hd_struct *part;

		/* set /dev/block/mmcblk0 Read-only */
		md->disk->part0.policy = 1;

		disk_part_iter_init(&piter, md->disk, DISK_PITER_INCL_EMPTY);
		while ((part = disk_part_iter_next(&piter))) {
			/* set custom partition(/dev/block/mmcblk0p4 Read-only */
			if (part->partno == CUSTOM_PARTNO) {
				part->policy = 1;
			}
		}
