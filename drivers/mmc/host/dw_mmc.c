...
static struct dw_mci *tp_host = NULL;
...
static void init_tp_monitor(void)
{
	if (tp_host && tp_host->pdata->tp_mon_tbl)
		tp_host->transferred_cnt = 0;
}
...
static void dw_mci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->clock) {
		...
		pm_qos_update_request(&slot->host->pm_qos_int, INT_QOS);
...
static void dw_mci_command_complete(struct dw_mci *host, struct mmc_command *cmd)
{
	...
	if (cmd->error) {
		...
		dev_err(&host->dev, ": CMD%d error(0x%08x) status(0x%08x)\n",
...
static irqreturn_t dw_mci_interrupt(int irq, void *dev_id)
{
	...
	do {
		...
		if(irq == 107) {
			static int cnt = 0;
			if(cnt >= 10000) {
				printk("[test] status=0x%x, pending=0x%x\n",status,pending);
				printk("[test] INTMASK=0x%x, IDSTS=0x%x IDINTEN=0x%x\n",mci_readl(host, INTMASK),mci_readl(host, IDSTS),mci_readl(host, IDINTEN));
				cnt=0;
			}
			cnt++;
		}
...
#if defined(CONFIG_ARM_EXYNOS5410_BUS_DEVFREQ) && defined (CONFIG_MMC_CLKGATE)
...
static void dw_mci_tp_mon(struct work_struct *work)
{
	...
	dev_dbg(&host->dev, "prepare: %d byte/s mif=%d cpu=%d\n",
...
int __devinit dw_mci_probe(struct dw_mci *host)
{
	...
	if (host->pdata->tp_mon_tbl) {
		...
		tp_host = host;
#endif
		INIT_DELAYED_WORK_DEFERRABLE(&host->tp_mon, dw_mci_tp_mon);
		pm_qos_add_request(&host->pm_qos_mif, PM_QOS_BUS_THROUGHPUT, 0);
		pm_qos_add_request(&host->pm_qos_cpu, PM_QOS_CPU_FREQ_MIN, 0);
	}
...
int dw_mci_resume(struct dw_mci *host)
{
	...
	if (host->dma_ops && host->dma_ops->init)
