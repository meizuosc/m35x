...
struct	thermal_trip_point_conf {
	...
	int low_trip_val[MAX_TRIP_COUNT];
	int normal_trip_val[MAX_TRIP_COUNT];
...
struct	thermal_cooling_conf {
	...
	int freq_clip_count;
	struct freq_clip_table low_freq_data[MAX_TRIP_COUNT];
	int low_mode;
	struct freq_clip_table normal_freq_data[MAX_TRIP_COUNT];
	int normal_mode;
...
static int exynos_set_boost_mode(struct thermal_zone_device *thermal, unsigned int boost_mode)
{
	...
	for (i = 0; i < clip_count; i++) {
		/* switch the frequency clipping table , boost_freq data <-> freq_data */
...
static int exynos_set_tmu_mode(struct thermal_zone_device *thermal, unsigned int mode)
{
	int i;
	int temp;
	int trip_count, clip_count;
	...
	case 0:
		/* update trip point */
		for (i = 0; i < trip_count; i++) {
			temp = th_zone->sensor_conf->trip_data.low_trip_val[i];
			th_zone->sensor_conf->trip_data.trip_val[i] = temp;
			exynos_set_trip_temp(thermal, i, temp);
			}

		/* update freq_data */
		for (i = 0; i < clip_count; i++) {
			th_zone->sensor_conf->cooling_data.freq_data[i] =
				th_zone->sensor_conf->cooling_data.low_freq_data[i];
		}
		th_zone->sensor_conf->cooling_data.low_mode = 1;
		th_zone->sensor_conf->cooling_data.normal_mode = 0;
		th_zone->sensor_conf->cooling_data.boost_mode = 0;
		break;
	case 1:
		/* update trip point */
		for (i = 0; i < trip_count; i++) {
			temp = th_zone->sensor_conf->trip_data.normal_trip_val[i];
			th_zone->sensor_conf->trip_data.trip_val[i] = temp;
			exynos_set_trip_temp(thermal, i, temp);
		}
		...
		for (i = 0; i < clip_count; i++) {
			th_zone->sensor_conf->cooling_data.freq_data[i] =
				th_zone->sensor_conf->cooling_data.normal_freq_data[i];
		}
		th_zone->sensor_conf->cooling_data.low_mode = 0;
		th_zone->sensor_conf->cooling_data.normal_mode = 1;
		th_zone->sensor_conf->cooling_data.boost_mode = 0;
		break;
	case 2:
		/* update trip point */
		for (i = 0; i < trip_count; i++) {
			temp = th_zone->sensor_conf->trip_data.boost_trip_val[i];
			th_zone->sensor_conf->trip_data.trip_val[i] = temp;
			exynos_set_trip_temp(thermal, i, temp);
		}
		...
		th_zone->sensor_conf->cooling_data.low_mode = 0;
		th_zone->sensor_conf->cooling_data.normal_mode = 0;
		th_zone->sensor_conf->cooling_data.boost_mode = 1;
		break;
	default:
		break;
	}
...
static struct thermal_zone_device_ops const exynos_dev_ops = {
	...
	.set_tmu_mode = exynos_set_tmu_mode,
...
static int exynos_tmu_initialize(struct platform_device *pdev, int id)
{
	...
	if (data->soc == SOC_ARCH_EXYNOS4) {
		...
		rising_threshold = threshold_code;
		falling_threshold = rising_threshold - 40; //set the cold_temp 20;
...
static int exynos_tmu_read(struct exynos_tmu_data *data)
{
	...
	clk_disable(data->clk);
	mutex_unlock(&data->lock);
	/*if all temp below 0 maybe is really cold */
	if (j == 0) {
		max = min = 0;
	}
/*if max and min have a big diffrent something wrong was happend. max or min is wrong ?*/
	if (max - min > INVALID_VALUE) {
		for (i = 0; i < j; i++) {
			if (alltemp[i] != max && (max - alltemp[i] > INVALID_VALUE)) {
				 max_wrong++;
			}
		}
		/*if the max value is not only bigger than the min value the max value is wrong */
		if (max_wrong >= 2) {
			for (i = 0; i < j; i++) {
				if (alltemp[i] == max) {
					alltemp[i] = 0;
			...
			/*if there is only one max value set the average value as max*/
			if (max_count == 1) {
				max = sum /(j - max_count);
			}
			/*if there are two max value choose "min is wrong"*/
			else {
				min = max;
			}
		/*if the max value is only bigger than min value the min is wrong*/
		} else {
			min = max;
		}
	}
...
#ifdef CONFIG_ARM_EXYNOS5410_BUS_DEVFREQ
	if (old_hot >= TMU_95)
		pr_info("[TMU] POLLING: temp=%d, int=%d, mif=%d, hot_event(for aref)=%d\n",
			max, g_intfreq, g_miffreq, hot_event);
#if 0
...
#if CONFIG_ARM_EXYNOS5410_BUS_DEVFREQ
...
int is_high_temp(void)
{
	u8 temp_code;
	int temp, i, max = INT_MIN;
	struct exynos_tmu_data *data;
...
static int exynos_power_mode_notifier(struct notifier_block *notifier,
				unsigned long power_mode_event, void *v)
{
	switch (power_mode_event) {
	case POWER_MODE_0:
		exynos_set_tmu_mode(th_zone->therm_dev, 0);
		break;
	case POWER_MODE_1:
		exynos_set_tmu_mode(th_zone->therm_dev, 1);
		break;
	case POWER_MODE_2:
		exynos_set_tmu_mode(th_zone->therm_dev, 2);
		break;
	}

	return NOTIFY_OK;
}
static struct notifier_block exynos_power_mode_nb = {
	.notifier_call = exynos_power_mode_notifier,
};
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < EXYNOS_TMU_COUNT; i++)
		len += snprintf(&buf[len], THERMAL_NAME_LENGTH, "sensor%d : %ld\n", i, temp[i]);
...
static int __devinit exynos_tmu_probe(struct platform_device *pdev)
{
	...
	if (__raw_readl(EXYNOS_INFORM5) == 0x03030303) {
		HOT_95 = 75;
		HOT_109 = 84;
		HOT_110 = 85;
	} else {
	...
	for (i = 0; i < exynos_sensor_conf.trip_data.trip_count; i++) {
		exynos_sensor_conf.trip_data.trip_val[i] =
			pdata->threshold + pdata->normal_trigger_levels[i];
		exynos_sensor_conf.trip_data.low_trip_val[i] =
			pdata->threshold + pdata->low_trigger_levels[i];
		exynos_sensor_conf.trip_data.normal_trip_val[i] =
			pdata->threshold + pdata->normal_trigger_levels[i];
	...
	exynos_sensor_conf.cooling_data.normal_mode = 1;	/*init as normal mode*/
	exynos_sensor_conf.cooling_data.low_mode = 0;

	for (i = 0; i < pdata->freq_tab_count; i++) {
		exynos_sensor_conf.cooling_data.freq_data[i].freq_clip_max =
					pdata->normal_freq_tab[i].freq_clip_max;
		exynos_sensor_conf.cooling_data.freq_data[i].temp_level =
					pdata->normal_freq_tab[i].temp_level;
		exynos_sensor_conf.cooling_data.freq_data[i].mask_val = cpu_all_mask;

		exynos_sensor_conf.cooling_data.low_freq_data[i].freq_clip_max =
					pdata->low_freq_tab[i].freq_clip_max;
		exynos_sensor_conf.cooling_data.low_freq_data[i].temp_level =
					pdata->low_freq_tab[i].temp_level;
		exynos_sensor_conf.cooling_data.low_freq_data[i].mask_val = cpu_all_mask;

		exynos_sensor_conf.cooling_data.normal_freq_data[i].freq_clip_max =
					pdata->normal_freq_tab[i].freq_clip_max;
		exynos_sensor_conf.cooling_data.normal_freq_data[i].temp_level =
					pdata->normal_freq_tab[i].temp_level;
		exynos_sensor_conf.cooling_data.normal_freq_data[i].mask_val = cpu_all_mask;
	...
	power_mode_register_notifier(&exynos_power_mode_nb);
