...
static DEVICE_ATTR(boost_mode, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP,
		boost_mode_show, boost_mode_store);
...
{
	int count, ret = 0;
	static int set_polling_count = 0;
	long temp, trip_temp;
		...
		case THERMAL_TRIP_ACTIVE:
			list_for_each_entry(instance, &tz->cooling_devices,
				...
				if (temp >= trip_temp) {
					...
					set_polling_count = 0;/*record the count again*/
				} else {
					set_polling_count++;
					cdev->ops->set_cur_state(cdev, 0);
					if(set_polling_count > 10) {
						tz->polling_delay = 2000;/*change the delay to 2s*/
						set_polling_count = 0;
					}
				}
			}
