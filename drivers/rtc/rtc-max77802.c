...
#define MAX77802_WAKE_SHIFT			3
#define MAX77802_WAKE_MASK			(1 << MAX77802_WAKE_SHIFT)
#define MAX77802_FREEZE_SHIFT                2
#define MAX77802_FREEZE_MASK			(1 << MAX77802_FREEZE_SHIFT)
	...
	enum MAX77802_RTC_OP op)
{
	...
	mask = data | MAX77802_FREEZE_MASK | MAX77802_WAKE_MASK;
	data &= ~MAX77802_FREEZE_MASK;  
	data |= MAX77802_WAKE_MASK;

	ret = max77802_update_reg(info->max77802->i2c, MAX77802_RTC_UPDATE0, data, mask);
	if (ret < 0)
...
static int max77802_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	alrm->enabled = 0;
	ret = max77802_read_reg(info->max77802->i2c, MAX77802_RTC_RTCAE1, &val);
...
*/
    ret = max77802_write_reg(info->max77802->i2c, MAX77802_RTC_RTCAE1, 0x00);
	if (ret < 0) {
...
/*
	if (!mutex_is_locked(&info->lock))
		dev_warn(info->dev, "%s: should have mutex locked\n", __func__);
*/
    ret = max77802_write_reg(info->max77802->i2c, MAX77802_RTC_RTCAE2, 0x00);
	if (ret < 0) {
		dev_err(info->dev, "%s: fail to write alarm reg(%d)\n",
				__func__, ret);
		goto out;
...
/*
	if (!mutex_is_locked(&info->lock))
		dev_warn(info->dev, "%s: should have mutex locked\n", __func__);
*/
	ret = max77802_write_reg(info->max77802->i2c, MAX77802_RTC_RTCAE1, 0x7F);
	if (ret < 0) {
		dev_err(info->dev, "%s: fail to write alarm reg(%d)\n",
				__func__, ret);
		goto out;
...
/*
	if (!mutex_is_locked(&info->lock))
		dev_warn(info->dev, "%s: should have mutex locked\n", __func__);
*/
	ret = max77802_write_reg(info->max77802->i2c, MAX77802_RTC_RTCAE2, 0x7F);
	if (ret < 0) {
		dev_err(info->dev, "%s: fail to write alarm reg(%d)\n",
				__func__, ret);
		goto out;
...
static int max77802_rtc_read_alarm_boot(struct device *dev,
				      struct rtc_wkalrm *alrm)
{
	struct max77802_rtc_info *info = dev_get_drvdata(dev);
	u8 data[RTC_NR_TIME];
	...
	ret = max77802_bulk_read(info->max77802->i2c, MAX77802_ALARM2_SEC, RTC_NR_TIME, data);
	if (ret < 0) {
		dev_err(info->dev, "%s:%d fail to read alarm reg(%d)\n",
				__func__, __LINE__, ret);
		goto out;
	...
	alrm->enabled = 0;
	ret = max77802_read_reg(info->max77802->i2c, MAX77802_RTC_RTCAE2, &val);
	for (i = 0; i < RTC_NR_TIME; i++) {
		if (val & (0x01<<i)) {
			alrm->enabled = 1;
			break;
...
static int max77802_rtc_read_update0(struct device *dev, unsigned int* enabled)
{
	struct max77802_rtc_info *info = dev_get_drvdata(dev);
	int ret, rtcwake = 0;
	u8 data;

	ret = max77802_read_reg(info->max77802->i2c, MAX77802_RTC_UPDATE0, &data);
	pr_info("%s: MAX77802_RTC_UPDATE0 0x%02x\n", __func__, data);

	rtcwake = (data >> 3) & 0x01; 
	*enabled = rtcwake;

	pr_info("rtcwake is %s\n", rtcwake ? "enable" : "disable");
...
static const struct rtc_class_ops max77802_rtc_ops = {
	...
	.read_rtcupdate0 = max77802_rtc_read_update0,
	...
	.read_alarm_boot = max77802_rtc_read_alarm_boot,
#endif
...
static int __devinit max77802_rtc_probe(struct platform_device *pdev)
{
	...
	ret = max77802_update_reg(info->max77802->i2c, MAX77802_RTC_UPDATE0, 0x00, 0x04);
	if (ret < 0)
...
static void max77802_rtc_shutdown(struct platform_device *pdev)
{
	...
	/* when shutdown, disable the alarm1, avoiding to the system poweron by alarm1*/
