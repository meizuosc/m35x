...
/*
* Not enable FS && FE interrupt since they are normal.
* Here enables all abnormal interrupts.
*/
	...
	*/
	{
		...
		.fmt_reg = S5PCSIS_CFG_FMT_USER(2),
	},
};
...
static void __s5pcsis_set_format(struct csis_state *state)
{
	...
	dev_dbg(&state->pdev->dev, "fmt: 0x%x, %d x %d\n",
...
static void s5pcsis_set_params(struct csis_state *state)
{
	...
	dev_dbg(&state->pdev->dev, "%s(), lanes is %d, hs_settle "
		"is %d, alignment is %d\n", __func__,
		pdata->lanes, pdata->hs_settle, pdata->alignment);
	...
	pr_info("%s(), Using EXTCLK\n", __func__);
	val |= S5PCSIS_CTRL_WCLK_EXTCLK;
	s5pcsis_write(state, S5PCSIS_CTRL, val);
	#else
	/*
	* Using PCLK to cope with FLITE overflow.
	*/
	pr_info("%s(), Using PCLK\n", __func__);
	val &= ~S5PCSIS_CTRL_WCLK_EXTCLK;
	s5pcsis_write(state, S5PCSIS_CTRL, val);
	#endif
	...
	struct v4l2_mbus_framefmt *mf)
{
	...
	if (csis_fmt == NULL) {
		pr_warn("%s(), WARN!find_csis_format() failed, using default value\n",
			__func__);
			   ...
			   struct v4l2_subdev_format *fmt)
{
	...
	dev_dbg(&state->pdev->dev, "%s() +++++, PAD: %d",
		__func__, fmt->pad);
			   ...
			   struct v4l2_subdev_format *fmt)
{
	...
	dev_dbg(&state->pdev->dev, "%s() +++++", __func__);
...
static irqreturn_t s5pcsis_irq_handler(int irq, void *dev_id)
{
	...
	if (err_print_cnt < 20) {
		v4l2_info(&state->sd, "%s :cnt:%d error : 0x%x\n",
			__func__, err_print_cnt, val);
		err_print_cnt ++;
	}
...
static int __devinit s5pcsis_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev,"%s(), lanes: %d, alignment: %d, "
		"hs_settle: %d, fixed_phy_vdd: %d\n",
		__func__, pdata->lanes, pdata->alignment,
		pdata->hs_settle, pdata->fixed_phy_vdd);
