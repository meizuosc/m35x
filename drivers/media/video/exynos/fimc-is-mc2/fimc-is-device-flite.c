	...
	struct fimc_is_video_ctx *video_ctx)
{
	...
	if (enable) {
		...
		flite_hw_set_dma_fmt(flite_reg_base, video_ctx);
	} else {
		cfg |= FLITE_REG_CIGCTRL_ODMA_DISABLE;
	}
	...
	struct fimc_is_video_ctx *video_ctx)
{
	...
	flite_hw_set_cam_source_size(mipi_reg_base, f_frame, video_ctx);
	...
	if (test_bit(bdone, &flite->state)) {
		...
		if (frame) {
			...
			if (!work_pending(&flite->work_queue)) {
				...
				buffer_done(flite->video_ctx, index);
				err("work pending, %d frame drop, instance(%d)",
					index, sensor->instance);
	...
	if (status) {
		if (status == (3<<4)) {
			...
			if (flite->sw_checker) {
				...
				if (instance == 0)
					*notify_fcount_sen0 = atomic_read(&flite->fcount);
				else if (instance == 1)
					*notify_fcount_sen1 = atomic_read(&flite->fcount);
	...
	u32 data)
{
	...
	if (channel == FLITE_ID_A) {
		...
		this->regs = (unsigned long)S5P_VA_FIMCLITE2;
...
int fimc_is_flite_remove(struct fimc_is_device_flite *this)
{
	int ret = 0;

	if (this->probed == 1) {
		if (this->regs == (unsigned long)S5P_VA_FIMCLITE0)
			free_irq(IRQ_FIMC_LITE0, this);
		else if (this->regs == (unsigned long)S5P_VA_FIMCLITE1)
			free_irq(IRQ_FIMC_LITE1, this);
		else if (this->regs == (unsigned long)S5P_VA_FIMCLITE2)
			free_irq(IRQ_FIMC_LITE2, this);
		else
			err("unmatched reg number : %d",this->regs);
	...
	struct fimc_is_video_ctx *video_ctx)
{
	...
	flite_hw_set_start_addr(this->regs, 0, video_ctx->buf_dva[0][0]);
	...
	flite_hw_set_start_addr(this->regs, 1, video_ctx->buf_dva[1][0]);
	...
	/*flite_hw_set_use_buffer(this->regs, 0);*/
	flite_hw_set_output_dma(this->regs, true, video_ctx);
	flite_hw_set_output_local(this->regs, false);

	start_fimc_lite(this->regs, frame, video_ctx);
...
int fimc_is_flite_stop(struct fimc_is_device_flite *this, bool wait)
{
	...
	ret = wait_event_timeout(this->wait_queue,
		(!wait || test_bit(FIMC_IS_FLITE_LAST_CAPTURE, &this->state)),
...
int fimc_is_flite_set_clk(int channel, struct fimc_is_core *core)
{
	int ret = 0;
	char mipi[20];
	char flite[20];
	...
	sprintf(mipi, "gscl_wrap%d", channel);

	if (soc_is_exynos5410()) {
		sprintf(flite, "gscl_flite%d", channel);
	} else if(soc_is_exynos5250()){
		switch (channel) {
		case FLITE_ID_A:
			sprintf(flite, "sclk_cam%d", channel);
			break;
		case FLITE_ID_B:
			sprintf(flite, "sclk_bayer");
			break;
		default:
			err("Bad FLITE channel(%d)\n", channel);
			break;
		}
	}
	...
	if (IS_ERR(mipi_ctrl)) {
		err("%s : clk_get(%s) failed\n",
			__func__, mipi);
		ret =  PTR_ERR(mipi_ctrl);
		goto exit;
	}
...
int fimc_is_flite_put_clk(int channel, struct fimc_is_core *core)
{
	int ret = 0;
	char mipi[20];
	char flite[20];
