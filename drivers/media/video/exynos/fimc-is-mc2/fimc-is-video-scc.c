...
static int fimc_is_scc_video_close(struct file *file)
{
	...
	fimc_is_video_close(video, &scc->framemgr);
					...
					struct v4l2_requestbuffers *buf)
{
	...
	struct fimc_is_subdev *scc = &ischain->scc;
	struct fimc_is_subdev *leader = scc->leader;
	...
	ret = fimc_is_video_reqbufs(video_ctx, &scc->framemgr, buf);
	if (ret)
...
static int fimc_is_scc_queue_setup(struct vb2_queue *vq,
...
static inline void fimc_is_scc_wait_prepare(struct vb2_queue *q)
{
}

static inline void fimc_is_scc_wait_finish(struct vb2_queue *q)
{
}
...
static void fimc_is_scc_buffer_queue(struct vb2_buffer *vb)
{
	...
	fimc_is_video_buffer_queue(video_ctx, vb, &scc->framemgr);
...
static int fimc_is_scc_buffer_finish(struct vb2_buffer *vb)
{
	...
	ret = fimc_is_subdev_buffer_finish(scc, vb->v4l2_buf.index);
