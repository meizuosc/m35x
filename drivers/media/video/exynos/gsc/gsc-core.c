	...
	if (frame->fmt->num_planes == 1) {
		...
		case 3:
			if (frame->fmt->pixelformat == V4L2_PIX_FMT_YVU420) {
				addr->cb = (dma_addr_t)(addr->y + frame->f_width *
					(frame->crop.top + frame->crop.height));
				addr->cr = (dma_addr_t)(addr->cb + (frame->f_width >> 1) *
					((frame->crop.top + frame->crop.height) >> 1));
			} else {
				addr->cb = (dma_addr_t)(addr->y + pix_size);
...
static int gsc_probe(struct platform_device *pdev)
{
	...
	if (soc_is_exynos5410()) {
		/* Get Gscaler clock */
			gsc->clock[CLK_CHILD] =
			clk_get(NULL, gsc_clocks[CLK_CHILD]);
		...
		gsc->clock[CLK_PARENT] =
			clk_get(NULL, gsc_clocks[CLK_PARENT]);
	...
	if (gsc->id == 3) {
		dev_info(&pdev->dev, "register subdev nodes\n");
		ret = v4l2_device_register_subdev_nodes(&gsc->mdev[MDEV_OUTPUT]->v4l2_dev);
		if (ret) {
			dev_err(&pdev->dev, "failed to make nodes for subdev\n");
			return ret;
		}
		ret = v4l2_device_register_subdev_nodes(&gsc->mdev[MDEV_CAPTURE]->v4l2_dev);
		if (ret) {
			dev_err(&pdev->dev, "failed to make nodes for subdev\n");
			return ret;
		}
...
static int gsc_resume(struct device *dev)
{
	...
	} else if (gsc_cap_opened(gsc) && soc_is_exynos5410()) {
