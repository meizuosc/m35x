...
#if defined(CONFIG_ARM_EXYNOS5410_BUS_DEVFREQ)
static struct pm_qos_request exynos5_flite_qos_dev;
static struct pm_qos_request exynos5_flite_qos_mem;
#endif
#ifdef CONFIG_ARM_EXYNOS_IKS_CPUFREQ
static struct pm_qos_request exynos5_flite_qos_cpufreq;
#define CPUFREQ_LIMIT_FLITE 600000
#endif
...
static struct flite_fmt flite_formats[] = {
	{
		...
		*/
		...
		/*
		* Noted by qudao, Must use user format
		*/
		.fmt_reg	= FLITE_REG_CIGCTRL_USER(2),
		.is_yuv		= 0,
	},
};
	...
	if (enable) {
		...
		if (!(flite->output & FLITE_OUTPUT_MEM)) {
			...
			if (cam->use_isp) {
				...
				* Qudao: Need this?????
				*/
				flite_hw_set_output_isp(flite, true);
			}
			...
			flite_hw_set_output_isp(flite, false);
			#if 1
			int_src = FLITE_REG_CIGCTRL_IRQ_OVFEN0_ENABLE |
				FLITE_REG_CIGCTRL_IRQ_LASTEN0_ENABLE |
				...
				FLITE_REG_CIGCTRL_IRQ_STARTEN0_DISABLE;
			#else
			pr_info("%s(), For test, disable overflow interrupt of FLITE\n",
				__func__);
			int_src = FLITE_REG_CIGCTRL_IRQ_OVFEN0_DISABLE|
				FLITE_REG_CIGCTRL_IRQ_LASTEN0_ENABLE |
				FLITE_REG_CIGCTRL_IRQ_ENDEN0_ENABLE |
				FLITE_REG_CIGCTRL_IRQ_STARTEN0_DISABLE;
			#endif
		...
		/*
		* Moved here by qudao.
		* Since I reckon FLITE_OUTPUT_GSC also need it.
		*/
		#if 1
		flite_hw_set_output_frame_count_seq(flite,
				flite->reqbufs_cnt);
		#else
		pr_info("%s(), FOR TEST, set 0 to count_seq\n", __func__);
		flite_hw_set_output_frame_count_seq(flite, 0);
		#endif
...
static irqreturn_t flite_irq_handler(int irq, void *priv)
{
	...
	if (!test_bit(FLITE_ST_POWER, &flite->state)) {
		pr_err("%s(), !!!Err! FLITE_ST_POWER is off!!! Dump regs!\n", __func__);
		flite_hw_dump_regs(flite);
		return IRQ_NONE;
	}
	...
	case FLITE_REG_CISTATUS_IRQ_SRC_OVERFLOW:
		...
		/*
		* Add by qudao according to M040
		*/
		flite_hw_clear_overflow_flag(flite);
		flite_err("overflow generated, quit irq handler now");
		#else
		/*
		* Intent to track overflow phenomenon.
		*/
		panic("FLITE OVERFLOW OCCURRED!!!");
		#endif
		goto unlock;
		//break;
	...
	if (flite->output & FLITE_OUTPUT_MEM) {
		if (!list_empty(&flite->active_buf_q)) {
			...
			pr_debug("done_index : %d", buf->vb.v4l2_buf.index);

			#ifdef USING_FLOW_CONTROL
			pr_debug("%s(), set buf %d Unavaliable to flite\n", __func__,
				buf->vb.v4l2_buf.index);
			flite_hw_set_unuse_buffer(flite, buf->vb.v4l2_buf.index);
			#endif
		}
		if (!list_empty(&flite->pending_buf_q)) {
			pr_info("%s(), Moving pending_buf to active_buf\n", __func__);
		...
		if (flite->active_buf_cnt == 0) {
			pr_debug("%s(), active_buf_cnt is 0\n", __func__);
			#ifdef USING_FLOW_CONTROL
			if (test_bit(FLITE_ST_STREAM, &flite->state)) {
				flite_hw_disable_last_irq(flite);
				pr_debug("%s(), stop capture\n", __func__);			
				flite_hw_set_capture_stop(flite);
				clear_bit(FLITE_ST_STREAM, &flite->state);
			}
			#endif
			     ...
			     enum v4l2_subdev_format_whence which)
{
	...
	if (ffmt == NULL) {
		dev_warn(&flite->pdev->dev, "%s(), find_flite_format() failed, "
			"use default vaule \n", __func__);
			    ...
			    const struct media_pad *remote, u32 flags)
{
	...
	case FLITE_PAD_SINK | MEDIA_ENT_T_V4L2_SUBDEV:
		if (flags & MEDIA_LNK_FL_ENABLED) {
			if (flite->input != FLITE_INPUT_NONE) {
				flite_err("link is busy, the existing input is %d", flite->input);
				return -EBUSY;
			}
			...
			flite_info("sink link enabled, set input to %d(2: csis)", flite->input);
		} else {
	...
	case FLITE_PAD_SOURCE_CAMCORD | MEDIA_ENT_T_V4L2_SUBDEV:
		if (flags & MEDIA_LNK_FL_ENABLED) {
			flite_info("source link(output2GSC) enabled");
			flite->output |= FLITE_OUTPUT_GSC;
		} else {
			flite_info("source link(output2GSC) disabled");
			flite->output &= ~FLITE_OUTPUT_GSC;
		}
		break;
	case FLITE_PAD_SOURCE_MEM | MEDIA_ENT_T_DEVNODE:
		if (flags & MEDIA_LNK_FL_ENABLED) {
			flite_info("source link(output2MEM) enabled");
			flite->output |= FLITE_OUTPUT_MEM;
		} else {
			flite_info("source link(output2MEM) disabled");
			flite->output &= ~FLITE_OUTPUT_MEM;
		}
		break;
	default:
...
static void flite_pipeline_prepare(struct flite_dev *flite, struct media_entity *me)
{
	...
	flite_dbg("flite->pipeline.sensor : 0x%p, subdev name: %s",
		flite->pipeline.sensor, flite->pipeline.sensor->name);
	flite_dbg("flite->pipeline.csis : 0x%p", flite->pipeline.csis);
}

#ifndef CONFIG_VIDEO_MS2R
...
#else
extern void flite_set_cam_clock(struct flite_dev *flite, bool on);
#endif
...
#else
extern int __subdev_set_power(struct v4l2_subdev *sd, int on);
#endif
...
static int flite_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	dev_info(&flite->pdev->dev, "%s():V4l2 control ID =0x%08x, val = %d\n",
	...
	case V4L2_CID_CAMERA_FOCUS_WINDOW:
		dev_info(&flite->pdev->dev, "Try s_ctrl on sensor subdev\n");
		ctrl_con.id = ctrl->id;
		...
		break;
	default:
		dev_err(&flite->pdev->dev, "%s(), Unkown s_ctrl ID: 0x%08x\n",
			__func__, ctrl->id);
		return -EINVAL;
	}
...
static int flite_g_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	switch (ctrl->id) {
	case V4L2_CID_CACHEABLE:
		/*Qudao: why this resembles flite_s_ctrl()????*/
		user_to_drv(flite->flite_ctrls.cacheable, ctrl->val);
		break;
	case V4L2_CID_CAMERA_WHITE_BALANCE:
	case V4L2_CID_CAMERA_BRIGHTNESS:
	case V4L2_CID_CAMERA_FOCUS_WINDOW:
		dev_info(&flite->pdev->dev, "Try g_ctrl on sensor subdev\n");
		ctrl_con.id = ctrl->id;

		ret = v4l2_subdev_call(p->sensor, core, g_ctrl, &ctrl_con);
		if (ret < 0 && ret != -ENOIOCTLCMD)
			return -EINVAL;
		ctrl->cur.val=ctrl_con.value;
		break;
	default:
		dev_err(&flite->pdev->dev, "%s(), Unkown g_ctrl ID: 0x%08x\n",
			__func__, ctrl->id);
		return -EINVAL;
	}
	return 0;
...
static const struct v4l2_ctrl_config flite_custom_ctrl[] = {
	{
		...
		.name = "white balance ioctl",
		.type = V4L2_CTRL_TYPE_INTEGER,
		.flags = V4L2_CTRL_FLAG_SLIDER,
		.min = 0,
		/*
		* This should be external ISP specific.
		*/
		.max = 0xFFFF,
		.step = 1,
		.def = 0,
		...
		*/
		...
		.ops = &flite_ctrl_ops,
		.id = V4L2_CID_CAMERA_FOCUS_WINDOW,
		.name = "focus window ioctl",
		.type = V4L2_CTRL_TYPE_INTEGER,
		.flags = V4L2_CTRL_FLAG_SLIDER,
		.min = 0,
		/*
...
static int flite_ctrls_create(struct flite_dev *flite)
{
	...
	v4l2_ctrl_handler_init(&flite->ctrl_handler, ARRAY_SIZE(flite_custom_ctrl));
	...
	flite->flite_ctrls.focuswindow = v4l2_ctrl_new_custom(&flite->ctrl_handler,
...
#else
static int inline flite_ctrls_create(struct flite_dev *flite)
{
	flite_dbg("do Nothing since we using VIDIOC_S_CTRL instead\n");
	return 0;
}

static void inline flite_ctrls_delete(struct flite_dev *flite)
{
	flite_dbg("do Nothing since we using VIDIOC_S_CTRL instead\n");
}
#endif
...
static int flite_open(struct file *file)
{
	...
	#ifdef USING_FLOW_CONTROL
	pr_info("%s(), using flow control\n", __func__);
	#else
	pr_info("%s(), NOT using flow control\n", __func__);
	#endif
...
#if defined(CONFIG_ARM_EXYNOS5410_BUS_DEVFREQ)
	pm_qos_add_request(&exynos5_flite_qos_dev, PM_QOS_DEVICE_THROUGHPUT, DVFS_L1);
	pm_qos_add_request(&exynos5_flite_qos_mem, PM_QOS_BUS_THROUGHPUT, DVFS_L0);
#endif
#ifdef CONFIG_ARM_EXYNOS_IKS_CPUFREQ
	pm_qos_add_request(&exynos5_flite_qos_cpufreq, PM_QOS_CPU_FREQ_MIN, CPUFREQ_LIMIT_FLITE);
#endif
...
static int flite_close(struct file *file)
{
	...
	flite_info("pid: %d, state: 0x%lx, refcnt is %d",
		task_pid_nr(current), flite->state, flite->refcnt);

	if (--flite->refcnt == 0) {
		/*
		* Qudao:
		* Here clears all bits of flite->state excpet FLITE_ST_SUBDEV_OPEN
		* && FLITE_ST_POWER.
		*/
		clear_bit(FLITE_ST_OPEN, &flite->state);
	...
	pm_qos_remove_request(&exynos5_flite_qos_mem);
#endif
#ifdef CONFIG_ARM_EXYNOS_IKS_CPUFREQ
	pm_qos_remove_request(&exynos5_flite_qos_cpufreq);
#endif

	pr_info("%s(), ------ returns with %d\n", __func__, ret);
	return ret;
}
...
#ifdef USING_ORIGINAL_POLL
	return vb2_poll(&flite->vbq, file, wait);
#else
	int ret;
	if (unlikely(list_empty(&flite->vbq.done_list))) {
		if (unlikely(debug)) {
			pr_info("%s(), invoke vb2_poll()\n", __func__);
		}
		ret = vb2_poll(&flite->vbq, file, wait);
		if (unlikely(debug)) {
			pr_info("%s(), ----- vb2_poll() returns 0x%x\n", __func__, ret);
		}
		return ret;
	} else {
		if (unlikely(debug)) {
			pr_info("%s(), return directly, flite->active_buf_cnt is %d\n",
				__func__, flite->active_buf_cnt);
		}
		return (V4L2_TYPE_IS_OUTPUT(flite->vbq.type)) ? POLLOUT | POLLWRNORM :
			POLLIN | POLLRDNORM;
	}
#endif
}
...
#else
	if (!test_bit(FLITE_ST_POWER, &flite->state)) {
		pr_err("%s(), Err: !Flite power status is down\n!", __func__);
		dump_stack();
		return -EINVAL;
	}
	/*
	* Sometimes the status bit FLITE_ST_RUN is of chaos,
	* consquently unconditional stop is needed and of no harm.
	*/
	pr_info("%s(), Force stop!!! flite_active: %d\n",
		__func__, flite_active(flite));
#endif
...
static void flite_buf_queue(struct vb2_buffer *vb)
{
	...
	pr_debug("%s(), active_buf_cnt is %d\n", __func__, flite->active_buf_cnt);
	...
	if (flite->active_buf_cnt < FLITE_MAX_OUT_BUFS) {
		...
		#ifdef USING_FLOW_CONTROL
		pr_debug("%s(), Set buf %d Available to flite\n", __func__, vb->v4l2_buf.index);
		flite_hw_set_use_buffer(flite, vb->v4l2_buf.index);
		#endif
	} else {
		pr_info("%s(), active_buf_cnt >= %d (reqbuf() count), so we need to \n"
			"add the buf(by user's qbuf()) to pending_buf_q\n",
			__func__, FLITE_MAX_OUT_BUFS);
		pending_queue_add(flite, buf);
	}

#ifdef USING_FLOW_CONTROL
	if (vb2_is_streaming(&flite->vbq) &&
		/*(flite->active_buf_cnt >= min_bufs) &&*/
		...
		if (!test_bit(FLITE_ST_STREAM, &flite->state)) {
			...
			flite_hw_set_capture_start(flite);
			set_bit(FLITE_ST_STREAM, &flite->state);
			//flite_hw_enable_last_irq(flite);
		}
	}
#else
	if (vb2_is_streaming(&flite->vbq) &&
		...
		if (!test_bit(FLITE_ST_STREAM, &flite->state)) {
			flite_info("FIMC-LITE h/w enable control");
			flite_hw_set_capture_start(flite);
			set_bit(FLITE_ST_STREAM, &flite->state);
		}
	}
...
#ifdef STORE_CAPTURE_RAWDATA
static int rawdata_file(void *data, unsigned long size)
{
	int ret = 0;
	struct file *fp = NULL;
	mm_segment_t old_fs;
	char *file_name_base = "/data/capture_rawdata";
	char file_name[strlen(file_name_base) + 6];
	loff_t pos = 0;
	static int index = 0;
	snprintf(file_name, sizeof(file_name), "%s%d", file_name_base, index);
	index++;
...
int flite_buf_finish(struct vb2_buffer *vb)
{
	struct flite_buffer *buf = container_of(vb, struct flite_buffer, vb);
	struct flite_dev *flite = vb2_get_drv_priv(vb->vb2_queue);
	void *vb2_kva;
	unsigned long vb2_size;

	if (!flite->cap_rawdata_done) {
		vb2_kva = vb2_plane_vaddr(&buf->vb, 0);
		vb2_size = vb2_get_plane_payload(&buf->vb, 0);
		pr_info("%s(), vb2_kva: %p, vb2_size: 0x%lx\n",
			__func__, vb2_kva, vb2_size);
		flite->cap_rawdata_done = 1;
		rawdata_file(vb2_kva, vb2_size);
	}
...
static int flite_try_fmt_mplane(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	/*
	* QUDAO:
	* This function requires the minimum value of  min_w and min_h to be 1,
	* NOT 0, but we don't, bug???
	* May be : min_w = min_h = 1, mod_y = 0
	*/
...
static int flite_s_fmt_mplane(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	frame->type=pix->reserved[0]; //by qlp
...
#else
int flite_s_fmt_mplane(struct file *file, void *fh, struct v4l2_format *f)
{
	struct flite_dev *flite = video_drvdata(file);
	struct flite_frame *frame;
	struct v4l2_pix_format_mplane *pix;
	/*
	* For manipulating ISP subdev.
	*/
	struct v4l2_mbus_framefmt *mbus_fmt = NULL;
	int ret = 0;
	...
	pix = &f->fmt.pix_mp;
	/*
	* pix->pixelformat is from userspace.
	*/
	frame->fmt = find_format(&pix->pixelformat, NULL, 0);
	if (!frame->fmt)
		return -EINVAL;
	...
	#ifdef STORE_CAPTURE_RAWDATA
	if (frame->fmt->code == V4L2_MBUS_FMT_SGRBG8_1X8 ||
		frame->fmt->code == V4L2_MBUS_FMT_SGRBG10_1X10 ||
		frame->fmt->code == V4L2_MBUS_FMT_JPEG_1X8) {
		pr_info("%s(), set rawdata_done to 0\n", __func__);
		flite->cap_rawdata_done = 0;
	} else {
		pr_info("%s(), set rawdata_done to 1\n", __func__);
		flite->cap_rawdata_done = 1;
	}
	#endif


	pr_info("%s(), Will invoking s_fmt of ISP subdev\n", __func__);
	mbus_fmt = kzalloc(sizeof(*mbus_fmt), GFP_KERNEL);
	if (!mbus_fmt) {
		pr_err("%s(), alloc memory fails\n", __func__);
		return -ENOMEM;
	}
	mbus_fmt->width = frame->o_width;
	mbus_fmt->height = frame->o_height;
	mbus_fmt->reserved[0] = pix->reserved[0];/*camera mode value*/
	mbus_fmt->colorspace = pix->colorspace;
	mbus_fmt->code =  frame->fmt->code;

	ret = v4l2_subdev_call(flite->pipeline.sensor, video, s_mbus_fmt, mbus_fmt);
	kfree(mbus_fmt);
	return ret;
}
#endif
...
static int flite_g_fmt_mplane(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	pix_mp->num_planes	= 1;
	pix_mp->reserved[0]	= frame->type; //by qlp
...
#else
	pr_info("%s(), request %d buffers. This should be imporved via "
		"V4L2_CID_CACHEABLE ioctl()\n", __func__, reqbufs->count);
...
static int flite_dqbuf(struct file *file, void *priv, struct v4l2_buffer *buf)
{
	struct flite_dev *flite = video_drvdata(file);
	int ret = 0;
	ret = vb2_dqbuf(&flite->vbq, buf, file->f_flags & O_NONBLOCK);
	buf->reserved = flite->active_buf_cnt;
	pr_debug("%s(), index: %d, member reserved's value: %d\n",
		__func__, buf->index, buf->reserved);
	return ret;
}
	...
	while (1) {
		...
		if (pad == NULL ||
		    media_entity_type(pad->entity) != MEDIA_ENT_T_V4L2_SUBDEV) {
		    	dev_warn(&flite->pdev->dev, "%s(), !!!!Get source pad failed!!!!\n",
					__func__);
			break;
		}
		...
		flite_dbg("src_width : %d, src_height : %d, src_code : 0x%x",
			src_fmt.format.width, src_fmt.format.height,
			src_fmt.format.code);
		flite_dbg("sink_width : %d, sink_height : %d, sink_code : 0x%x",
...
static int flite_streamon(struct file *file, void *priv, enum v4l2_buf_type type)
{
	...
	flite_dbg("Before invoking flite_link_validate()");
	ret = flite_link_validate(flite);
	if (ret) {
		pr_err("%s(), flite_link_validate() failed: %d\n",
			__func__, ret);
		return ret;
	}
	flite_dbg("Before invoking flite_hw_reset()");
	flite_hw_reset(flite);
	flite_dbg("After invoking flite_hw_reset()");
#if 1
	/*
	* This is from exynos5250
	*/
	init_waitqueue_head(&flite->irq_queue);

	INIT_LIST_HEAD(&flite->active_buf_q); //by qlp
	INIT_LIST_HEAD(&flite->pending_buf_q);

	if (test_bit(FLITE_ST_SUSPEND, &flite->state))
		clear_bit(FLITE_ST_SUSPEND, &flite->state);
#endif
	flite_dbg("Before invoking vb2_streamon()");
	ret = vb2_streamon(&flite->vbq, type);
	pr_info("%s(), ---------------, ret: %d\n", __func__, ret);
	return ret;
}
...
#else
extern int flite_enum_input(struct file *file, void *priv,
	struct v4l2_input *i);
extern int flite_s_input(struct file *file, void *priv, unsigned int i);
#endif
...
static const struct v4l2_ioctl_ops flite_capture_ioctl_ops = {
	...
	#if defined(CONFIG_VIDEO_MS2R) && defined(USING_VIDIOC_S_CTRL)
	.vidioc_s_ctrl			= flite_s_ioctrl,
	.vidioc_g_ctrl			= flite_g_ioctrl,
	#endif
};
		...
		struct exynos_isp_info *isp_info, int i)
{
	...
	if (IS_ERR_OR_NULL(cam_srclk)) {
		flite_err("failed to get cam source clk: %s", cam_srclk_name);
		return -ENXIO;
	}
		...
		int i)
{
	...
	WARN_ON(!isp_info->board_info);

	dev_info(&flite->pdev->dev, "%s(), bus_num: %d,"
		"client name: %s, client address: 0x%x\n",
		__func__, isp_info->i2c_bus_num,
		isp_info->board_info->type,isp_info->board_info->addr);

	adapter = i2c_get_adapter(isp_info->i2c_bus_num);
	if (!adapter) {
		dev_err(&flite->pdev->dev, "%s(), failed to get I2C adapter %d\n",
			__func__, isp_info->i2c_bus_num);
		return NULL;
	}
	...
	pr_info("%s(), sensor subdev sd address 0x%p, sd->host_priv is 0x%p\n",
		__func__, sd, sd->host_priv);
...
static int flite_register_video_device(struct flite_dev *flite)
{
	...
	*/
	...
	dev_info(&flite->pdev->dev, "%s(), We use ctrl_handler\n",
		__func__);
#else
	dev_info(&flite->pdev->dev, "%s(), We use VIDOC_S_CTRL instead\n",
		__func__);
#endif
...
static struct v4l2_subdev_video_ops flite_video_ops = {
	/*
	* The following except s_stream are never invoked???
	*/
...
static int flite_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "pdata: num_clients %d, active_cam_index %d\n",
		flite->pdata->num_clients,
		flite->pdata->active_cam_index);
	...
	flite_info("mdev = 0x%08x", (u32)flite->mdev);
