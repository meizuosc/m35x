...
#define USING_VIDIOC_S_CTRL
//#define STORE_CAPTURE_RAWDATA
#define USING_FLOW_CONTROL
//#define USING_ORIGINAL_POLL
/*
* This is used by HAL to retrieve the corresponding media_entity
*/
#define ISP_ENTITY_NAME "ms2r 0-001f"
...
enum flite_oprmode {
	/*
	* This represents preview && video.
	*/
	FLITE_OPRMODE_VIDEO = 0,
	/*
	* This represents taking picture.
	*/
	FLITE_OPRMODE_IMAGE = 1,
	FLITE_OPRMODE_MAX,
};
/**
...
extern int flite_s_ioctrl(struct file *file, void *fh,struct v4l2_control *c);
extern int flite_g_ioctrl(struct file *file, void *fh, struct v4l2_control *c);
#endif
/* fimc-reg.c */
...
static inline void flite_hw_get_int_src(struct flite_dev *dev, u32 *src)
{
	*src = readl(dev->regs + FLITE_REG_CISTATUS);
	pr_debug("%s(), read reg 0x40 's value is 0x%x\n",
		__func__, *src);
...
* Added by qudao
*/
static inline void flite_hw_clear_overflow_flag(struct flite_dev *dev)
{
	u32 val = readl(dev->regs + FLITE_REG_CIWDOFST);
	val |= (FLITE_REG_CIWDOFST_CLROVIY |
			FLITE_REG_CIWDOFST_CLROVFICB |
			FLITE_REG_CIWDOFST_CLROVFICR);
	writel(val, dev->regs + FLITE_REG_CIWDOFST);

	val = readl(dev->regs + FLITE_REG_CIWDOFST);
	val &= ~ (FLITE_REG_CIWDOFST_CLROVIY |
			FLITE_REG_CIWDOFST_CLROVFICB |
			FLITE_REG_CIWDOFST_CLROVFICR);
	writel(val, dev->regs + FLITE_REG_CIWDOFST);
