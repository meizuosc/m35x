...
static void hdmi_hpd_changed(struct hdmi_device *hdev, int state)
{
	...
	switch_set_state(&hdev->audio_switch_dev,
		((state && (edid_max_audio_channels(hdev) > 0)) ? 1 : 0));
#endif
...
static int __devinit hdmi_probe(struct platform_device *pdev)
{
    ...
    ret = gpio_request(EXYNOS5410_GPY6(3), "5V_PWREN");
    if (ret)
        printk("%s, failed to request EXYNOS5410_GPY6(3) for 5V_PWREN for HDMI hotplug\n",__func__);

    s3c_gpio_cfgpin(EXYNOS5410_GPY6(3), S3C_GPIO_OUTPUT);
...
#ifdef CONFIG_MACH_TF4
	hdmi_dev->audio_switch_dev.name = "hdmi_audio";
	switch_dev_register(&hdmi_dev->audio_switch_dev);
#endif
...
#ifdef CONFIG_MACH_TF4
	switch_dev_unregister(&hdmi_dev->audio_switch_dev);
#endif
