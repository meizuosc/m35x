...
inline void clear_work_bit(struct s5p_mfc_ctx *ctx)
{
	...
	spin_lock_irqsave(&dev->condlock, flag);
	clear_bit(ctx->num, &dev->ctx_work_bits);
	spin_unlock_irqrestore(&dev->condlock, flag);
}
...
static int s5p_mfc_open(struct file *file)
{
	...
	pr_info("%s instance no %d, num instance %d\n",__func__, ctx->inst_no, dev->num_inst);
	return ret;
	...
	if (!atomic_read(&dev->watchdog_run) &&
		...
		if (s5p_mfc_wait_for_done_ctx(ctx,
			...
			if (s5p_mfc_wait_for_done_ctx(ctx,
				...
				pr_info("##### instance number %d, num inst %d\n", ctx->inst_no,
						dev->num_inst);
...
out:
	...
	pr_info("--%s number of instance %d\n", __func__, dev->num_inst);
