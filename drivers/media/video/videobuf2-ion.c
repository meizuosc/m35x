...
static void vb2_ion_lock(struct vb2_ion_context *ctx)
{
	if (in_interrupt() || !preemptible())
		spin_lock(&ctx->spinlock);
	else
		mutex_lock(&ctx->lock);
}
static void vb2_ion_unlock(struct vb2_ion_context *ctx)
{
	if (in_interrupt() || !preemptible())
				  ...
				  unsigned long size, int write)
{
	...
	pr_debug("%s(), dma buf size 0x%x (Should NOT "
		"less than needed size 0x%lx)\n",
		__func__, dbuf->size, size);
				...
				unsigned long size, int write)
{
	...
	if (need_kaddr(ctx, size, buf->cached)) {
		...
		buf->kva = dma_buf_kmap(buf->dma_buf, buf->cookie.offset);
		if (!buf->kva) {
			dev_err(ctx->dev,
...
static void vb2_ion_put_userptr(void *mem_priv)
{
	...
	if (buf->kva) {
		dma_buf_kunmap(buf->dma_buf, buf->cookie.offset, buf->kva);
...
static void __local_cache_flush_all(void *p)
{
	flush_cache_all();
}
...
{
	preempt_disable();
	smp_call_function(&__local_cache_flush_all, NULL, 1);
	flush_cache_all();
	preempt_enable();
}
						...
						enum dma_data_direction dir)
{
	...
	if (!buf->kva) {
		...
		dmac_map_area(buf->kva + vb2cookie->offset + offset,
				size, dir);
	}
						...
						enum dma_data_direction dir)
{
	...
	if (!buf->kva) {
		...
		dmac_unmap_area(buf->kva + vb2cookie->offset + offset,
				size, dir);
	}
...
int vb2_ion_buf_prepare(struct vb2_buffer *vb)
{
	...
	for (i = 0; i < vb->num_planes; i++) {
		...
		if (!buf->kva)
			...
			dmac_map_area(buf->kva + buf->cookie.offset,
					buf->size, dir);
	}
...
int vb2_ion_buf_finish(struct vb2_buffer *vb)
{
	...
	for (i = 0; i < vb->num_planes; i++) {
		...
		if (!buf->kva)
			...
			dmac_unmap_area(buf->kva + buf->cookie.offset,
					buf->size, dir);
	}
