...
#ifdef CONFIG_HAS_WAKELOCK
	wake_lock(&bcm2079x_dev->nfc_wakelock);
#endif
	spin_lock_irqsave(&bcm2079x_dev->irq_enabled_lock, flags);
...
#ifdef CONFIG_HAS_WAKELOCK
		if (arg != 0)
			wake_unlock(&bcm2079x_dev->nfc_wakelock);
#endif
...
#ifdef CONFIG_HAS_WAKELOCK
	wake_lock_init(&bcm2079x_dev->nfc_wakelock, WAKE_LOCK_SUSPEND, "nfc_lock");
#endif
...
static int bcm2079x_remove(struct i2c_client *client)
{
	...
	misc_deregister(&bcm2079x_dev->bcm2079x_device);
#ifdef CONFIG_HAS_WAKELOCK
	wake_lock_destroy(&bcm2079x_dev->nfc_wakelock);
#endif
	mutex_destroy(&bcm2079x_dev->read_mutex);
...
static int bcm2709x_suspend(struct device *dev)
{
	unsigned long flags;
	struct bcm2079x_dev *bcm2079x_dev;

	bcm2079x_dev = dev_get_drvdata(dev);
	spin_lock_irqsave(&bcm2079x_dev->irq_enabled_lock, flags);
	if (bcm2079x_dev->irq_enabled) {
		disable_irq(bcm2079x_dev->client->irq);
		irq_set_irq_type(bcm2079x_dev->client->irq,
...
static int bcm2709x_resume(struct device *dev)
{
	unsigned long flags;
	struct bcm2079x_dev *bcm2079x_dev;
...
static const struct dev_pm_ops bcm2709x_pm_ops = {
	.suspend = bcm2709x_suspend,
	.resume  = bcm2709x_resume,
};
