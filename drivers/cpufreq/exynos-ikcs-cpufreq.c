 ...
 * change by chenshb: chenshb@meizu.com
 *	change boot cpufreq [800M~1000M] to 600M.
 *	because A7 600M performance better than A15 800M and save power.
 */
...
/* Current T/F - ARM Core Voltage Down */
unsigned int lp_volt_need = 0, prev_cur_freq = 0;
/* Current T/F - ARM Core Voltage Down */
...
cluster_type boot_cluster = CA_END;
 ...
 * optimize by chenshb@meizu.com.
 * remove the merge_index_table invalid options.
 */
static int cpufreq_merge_tables(void)
{
	int i;
	struct cpufreq_frequency_table *freq_table, *freq_table_a7, *freq_table_a15;
	unsigned int lenA7 = 0;
	unsigned int lenA15 = 0;
	unsigned int j = 0;
	unsigned int k = 0;
	int last_len = 0;

	freq_table_a7 = exynos_info[CA7]->freq_table;
	freq_table_a15 = exynos_info[CA15]->freq_table;

	for (i = 0; freq_table_a7[i].frequency != CPUFREQ_TABLE_END; i++) {
		if (freq_table_a7[i].frequency != CPUFREQ_ENTRY_INVALID)
			lenA7++;
	}

	for (i = 0; freq_table_a15[i].frequency != CPUFREQ_TABLE_END; i++) {
		if (freq_table_a15[i].frequency != CPUFREQ_ENTRY_INVALID)
			lenA15++;
	}

	last_len = lenA15 + lenA7 ;
	freq_table = kzalloc(sizeof(struct cpufreq_frequency_table) * (last_len+1), GFP_KERNEL);
	if (!freq_table)
		return -ENOMEM;
	merge_index_table = kzalloc(sizeof(unsigned int) * (last_len + 1), GFP_KERNEL);
	...
	for (i = 0; i < last_len; i++) {
		if (i < lenA15) {
			while (freq_table_a15[j].frequency == CPUFREQ_ENTRY_INVALID)
				j++;
			merge_index_table[i] = j;
			merge_freq_table[i].index = freq_table_a15[j].index;
			merge_freq_table[i].frequency = freq_table_a15[j++].frequency;
		} else {
			while (freq_table_a7[k].frequency == CPUFREQ_ENTRY_INVALID)
				k++;
			merge_index_table[i] = k;
			merge_freq_table[i].index = freq_table_a7[k].index;
			merge_freq_table[i].frequency = freq_table_a7[k++].frequency/2;
		}
	}
	merge_freq_table[last_len].frequency = CPUFREQ_TABLE_END;
	return 0;
}
			  ...
			  unsigned int relation)
{
	...
	/* Current T/F - ARM Core Voltage Down */
	if((do_switch == false) && (cur == CA7) && (lp_volt_need == 1)){
	...
	else if(do_switch && (cur == CA15)) {
		prev_cur_freq = target_freq;
		lp_volt_need = 1;
	}
	else if(do_switch && (cur == CA7)){
		volt = get_match_volt(ID_ARM, ACTUAL_FREQ(prev_cur_freq, CA15));
		volt = get_limit_voltage(volt);
		regulator_set_voltage(arm_regulator, volt, volt);
	}
	/* Current T/F - ARM Core Voltage Down */
...
#define TRANSITION_DOWN_LIMIT 1200000
#define REBOOT_LOCK_FREQ 400000
#define TRANSITION_DELAY_MS 500

static void exynos_cpufreq_reboot_lock_cpufreq(void)
{
	unsigned int target_freq = REBOOT_LOCK_FREQ;
	struct cpufreq_policy *policy;

	/* Lock minimal cpu freq at 400M */

	pm_qos_update_request(&boot_cpu_qos, target_freq);

	/* Lock maximal cpu freq at 400M, see below */

	/* If switch from a higher freq to a lower freq, such as 1.6G -> 400M, cpu will crash.
	 * To fix up such issue, switch to a transtion freq at first.
	 * Based on the same algorithm used for /sys/power/power_mode.
	 */
	policy = cpufreq_cpu_get(0);
	if (policy) {
		if (policy->cur > TRANSITION_DOWN_LIMIT)
			target_freq = TRANSITION_DOWN_LIMIT;
		cpufreq_cpu_put(policy);
	}

	pm_qos_update_request(&max_cpu_qos, target_freq);

	if (target_freq == TRANSITION_DOWN_LIMIT) {
		msleep(TRANSITION_DELAY_MS);
		pm_qos_update_request(&max_cpu_qos, REBOOT_LOCK_FREQ);
	}
}
				   ...
				   unsigned long code, void *_cmd)
{
	...
	/* Lock at 400M */
	pr_info("%s: Lock cpu freq at 400M\n", __func__);
	exynos_cpufreq_reboot_lock_cpufreq();
...
static int __init exynos_cpufreq_init(void)
{
	...
	pm_qos_update_request_timeout(&boot_cpu_qos, 600000, BOOT_CPUFREQ_LOCK_TIME * 1000);
	pm_qos_update_request_timeout(&max_cpu_qos, 600000, BOOT_CPUFREQ_LOCK_TIME * 1000);
...
late_initcall_sync(exynos_cpufreq_init);
