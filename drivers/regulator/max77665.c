...
struct max77665_data {
	...
	unsigned int saved_states[MAX77665_REG_MAX];
	bool enable;
};
...
struct class *charging;
EXPORT_SYMBOL_GPL(charging);
 ...
 */
int max77665_clear_interrupts(struct regmap *regmap)
{
	unsigned int val;
	int ret;
	ret = max77665_read_reg(regmap, 0x0e, &val);
					...
					int *reg, int *mask, int *pattern)
{
	...
	case MAX77665_FLASH_LED:
		...
		*pattern = MAX77665_FLASH_ENABLE_MASK << MAX77665_FLASH_ENABLE_SHIFT;
		pr_info("%s(), flash, reg:0x%x, mask:0x%x, pattern:0x%x\n",
			__func__, *reg, *mask, *pattern);
		break;
	case MAX77665_TORCH_LED:
		...
		*pattern = MAX77665_TORCH_ENABLE_MASK << MAX77665_TORCH_ENABLE_SHIFT;
		pr_info("%s(), torch, reg:0x%x, mask:0x%x, pattern:0x%x\n",
			__func__, *reg, *mask, *pattern);
		break;
...
static int max77665_reg_is_enabled(struct regulator_dev *rdev)
{
	...
	struct regmap *regmap = max77665->iodev->regmap;
	...
	ret = max77665_read_reg(regmap, reg, &val);
	if (ret)
		return ret;
...
static int max77665_reg_enable(struct regulator_dev *rdev)
{
	...
	return max77665_update_reg(regmap, reg, mask, pattern);
}
...
static int max77665_reg_disable(struct regulator_dev *rdev)
{
	...
	if (rid == MAX77665_FLASH_LED || rid == MAX77665_TORCH_LED) {
		ret = max77665_clear_interrupts(regmap);
		if (ret) return ret;
	}
...
static int max77665_charger_reg_enable(struct regulator_dev *rdev)
{
	...
	ret = max77665_update_reg(regmap, MAX77665_CHG_REG_CHG_CNFG_00, 0xF, 0x05);
...
static int max77665_charger_reg_disable(struct regulator_dev *rdev)
{
	...
	ret = max77665_update_reg(regmap, MAX77665_CHG_REG_CHG_CNFG_00, 0xF, 0x04);
...
static int max77665_reverse_reg_enable(struct regulator_dev *rdev)
{
	...
	ret = max77665_update_reg(regmap, MAX77665_CHG_REG_CHG_CNFG_00, 0xF, 0x0A);
...
static int max77665_reverse_reg_disable(struct regulator_dev *rdev)
{
	...
	ret = max77665_read_reg(regmap, MAX77665_CHG_REG_CHG_CNFG_00, &value);
	if ((value & 0x0f) == 0x05)
	    ret = max77665_update_reg(regmap, MAX77665_CHG_REG_CHG_CNFG_00, 0xF, 0x05);
	else 
	    ret = max77665_update_reg(regmap, MAX77665_CHG_REG_CHG_CNFG_00, 0xF, 0x04);
...
static int max77665_flash_set_current(struct regulator_dev *rdev, int cur)
{
	...
	/*set 2 led current*/
	for (i = 0; i < 2; i++) {
		pr_info("%s(), reg:0x%x, mask:0x%x, shift:0x%x, cur:0x%x\n",
			__func__, reg[i], mask[i], shift[i], cur);
		ret = max77665_update_reg(regmap, reg[i], mask[i] << shift[i], cur << shift[i]);
		if (ret)
			return ret;
	}
				...
				int min_uV, int max_uV)
{
	...
	case MAX77665_CHARGER:
		...
		ret = max77665_update_reg(regmap, reg, mask << shift, i << shift);
		break;
	case MAX77665_FLASH_LED:
	case MAX77665_TORCH_LED:
...
static int max77665_battery_get_current(struct regulator_dev *rdev)
{
	...
	ret = max77665_read_reg(regmap, MAX77665_CHG_REG_CHG_CNFG_02, &val);
	if (ret)
		return ret;
				...
				int min_uA, int max_uA)
{
	...
	ret = max77665_update_reg(regmap, MAX77665_CHG_REG_CHG_CNFG_02, 0x3F, reg);
	if (unlikely(ret)) {
					...
					unsigned *selector)
{
	...
	ret = max77665_update_reg(regmap, reg, mask << shift, val << shift); 
	*selector = val;
...
static int max77665_reg_disable_suspend(struct regulator_dev *rdev)
{
	...
	max77665_read_reg(regmap, reg, &max77665->saved_states[rid]);
...
static ssize_t charger_enabled_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	struct max77665_data *max77665 = dev_get_drvdata(dev);

	return sprintf(buf, "%d\n", max77665->enable);
}

static ssize_t charger_enabled_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	...
	max77665->enable = simple_strtol(buf, NULL, 10);
	if (!max77665->enable) {
		pr_info("disable charging for debug\n");
		ret = max77665_update_reg(regmap, MAX77665_CHG_REG_CHG_CNFG_00, 0xF, 0x00);
	} else {
		ret = max77665_update_reg(regmap, MAX77665_CHG_REG_CHG_CNFG_00, 0xF, 0x05);
		pr_info("enable charging for debug\n");
	}

	return count;
}

static DEVICE_ATTR(charger_enable, 0644, charger_enabled_show,
			charger_enabled_store);

static ssize_t show_led_regs_dump(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < MAX77665_PMIC_REG_END; i++) {
		max77665_read_reg(regmap, i, &val);
...
static DEVICE_ATTR(led_regs_dump, S_IRUGO|S_IWUSR|S_IWGRP,
			show_led_regs_dump, NULL);
...
static __devinit int max77665_pmic_probe(struct platform_device *pdev)
{
	...
	max77665->enable = true;
	charging = class_create(THIS_MODULE, "charging");

	max77665->dev = device_create(charging, max77665->dev->parent,
			0, max77665, "charger_dev");
	ret = device_create_file(max77665->dev, &dev_attr_charger_enable);
	if (ret < 0)
		goto err;

	/* No active discharge safeout1 and safeout2, disable safeout2 */
	max77665_update_reg(regmap, MAX77665_CHG_REG_SAFEOUT_CTRL, 0xB<<4, 0x0);

	pr_info("%s(), create sysfs\n", __func__);
	ret = sysfs_create_file(&pdev->dev.kobj, &dev_attr_led_regs_dump.attr);
	if (ret < 0) {
		pr_err("%s(), create led_regs_dump sysfs failed\n", __func__);
	}
