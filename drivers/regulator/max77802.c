...
static const struct voltage_map_desc *reg_voltage_map[] = {
	...
	[MAX77802_LDO22] = &pldo_voltage_map_desc,
...
unsigned int max77802_opmode_reg[][3] = {
	...
	{0x3, 0x0, 0x1},
	{0x3, 0x0, 0x0},
	{0x3, 0x0, 0x0}, /*BUCK8 INAND2.85V, must be controlled by XMMC0CDN*/
	{0x3, 0x0, 0x0},
	{0x3, 0x0, 0x0}, /* BUCK10 */
	/* 32KHZ */
	{0x1, 0x0, 0x0},
		...
		int *reg, int *mask, int *pattern)
{
	...
	if ((max77802_opmode_reg[rid][mode] == 0x0) && (rid != MAX77802_BUCK8))
		WARN(1, "rid[%s]: Not supported opmode\n",rdev->desc->name);
	...
	case MAX77802_LDO1 ... MAX77802_LDO15:
		...
		*pattern = max77802_opmode_reg[rid][mode] << MAX77802_OPMODE_SHIFT;
		break;
	case MAX77802_LDO17 ... MAX77802_LDO30:
...
static int max77802_reg_enable(struct regulator_dev *rdev)
{
	...
	pr_debug(PMIC_DEBUG "%s: id=%d, pattern=%x\n",
...
static int max77802_get_voltage(struct regulator_dev *rdev)
{
	...
	pr_debug(PMIC_REG_DEBUG "%s: id=%d, val=%x\n",
		__func__, rid, val);
		...
		int min_uV, int max_uV, unsigned *selector)
{
	...
	pr_debug(PMIC_REG_DEBUG "max77802: id=%d, org=%x, val=%x\n", rdev_get_id(rdev), org, i);
...
static int max77802_set_ramp_rate(struct i2c_client *i2c, int rate)
{
	...
	pr_debug(PMIC_DEBUG "%s: ramp_delay=%d, data=0x%x\n", __func__, ramp_delay, data);
...
static __devinit int max77802_pmic_probe(struct platform_device *pdev)
{
	...
	pr_debug(PMIC_DEBUG "%s: DEVICE ID=0x%x\n", __func__, data);

	max77802_read_reg(i2c, MAX77802_REG_PWRON, &data);
	pr_info("%s---------->PWRON: %x\n", __func__, data);

	/*Disable boost,not used*/
	max77802_read_reg(i2c, MAX77802_REG_BOOSTCTRL, &data);
	max77802_write_reg(i2c, MAX77802_REG_BOOSTCTRL, (data & (~(0x1<<1))));

	/*Enable rovs and fsrade for buck234*/
	max77802_update_reg(i2c, MAX77802_REG_BUCK2CTRL1, 0x3, 0x3);
	max77802_update_reg(i2c, MAX77802_REG_BUCK3CTRL1, 0x3, 0x3);
	max77802_update_reg(i2c, MAX77802_REG_BUCK4CTRL1, 0x3, 0x3);
	...
	for (i = 0; i < pdata->num_regulators; i++) {
		...
		if (desc) {
			...
			pr_debug(PMIC_DEBUG "%s: desc=%p, id=%d, n_vol=%d, max=%d, min=%d, step=%d\n",
	...
	if(1)
	{
		...
		{
			dev_err(max77802->dev, "failed to get regulator (0x%p).\n",regulator);
		}
	}
#endif
