...
struct arizona_extcon_info {
	...
	struct wake_lock switch_arizona_lock;
};
...
static irqreturn_t arizona_hpdet_irq(int irq, void *data)
{
	...
	/* Wait for hpdet finish */
	wake_lock_timeout(&info->switch_arizona_lock, msecs_to_jiffies(LOCK_TIMEOUT));
...
static int arizona_extcon_probe(struct platform_device *pdev)
{
	...
	info->dev = &pdev->dev;
	wake_lock_init(&info->switch_arizona_lock, WAKE_LOCK_SUSPEND, "switch_arizona_lock");
...
static int arizona_extcon_remove(struct platform_device *pdev)
{
	...
	wake_lock_destroy(&info->switch_arizona_lock);
	arizona_set_irq_wake(arizona, jack_irq_rise, 0);
	arizona_set_irq_wake(arizona, jack_irq_fall, 0);
	arizona_free_irq(arizona, ARIZONA_IRQ_HPDET, info);
	arizona_free_irq(arizona, ARIZONA_IRQ_MICDET, info);
