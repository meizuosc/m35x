...
static unsigned int exynos5410_clkdiv_top1[][3] = {
...
void resume_lock_int_timeout(unsigned long timeout_us)
{
	pr_info("%s @@@@@@@@ 400M\n", __func__);
	if (pm_qos_request_active(&exynos5_int_qos))
		pm_qos_update_request_timeout(&exynos5_int_qos, 400000, timeout_us);
}

void resume_lock_int(void)
{
	pr_debug("%s @@@@@@@@ 100M\n", __func__);
	if (pm_qos_request_active(&exynos5_int_qos))
		pm_qos_update_request(&exynos5_int_qos, 50000);
