				   ...
				   unsigned long code, void *_cmd)
{
	...
	if (type == code) {
		if (freq->new <= 200000)
			pm_qos_update_request(&exynos5_mif_qos, 100000);
		else if (freq->new <= 500000)
			pm_qos_update_request(&exynos5_mif_qos, 200000);
						...
						unsigned long event, void *v)
{
	...
	case TMU_95:
		...
		if (pm_qos_active(&min_mif_thermal_qos))
			pm_qos_remove(&min_mif_thermal_qos);
...
static __devinit int exynos5_busfreq_mif_probe(struct platform_device *pdev)
{
	...
	if (is_charging_mode())
		pm_qos_update_request_timeout(&boot_mif_qos, 800000, 4 * 1000);
	else
		pm_qos_update_request_timeout(&boot_mif_qos, 800000, BOOT_DEVFREQ_LOCK_TIME * 1000);
...
void resume_lock_mif_timeout(unsigned long timeout_us)
{
	pr_info("%s @@@@@@@@ 800M\n", __func__);
	if (pm_qos_request_active(&exynos5_mif_qos))
		pm_qos_update_request_timeout(&exynos5_mif_qos, 800000, timeout_us);
}

void resume_lock_mif(void)
{
	pr_debug("%s @@@@@@@@ 100M\n", __func__);
	if (pm_qos_request_active(&exynos5_mif_qos))
		pm_qos_update_request(&exynos5_mif_qos, 100000);
...
static int exynos5_busfreq_mif_suspend(struct device *dev)
{
	...
	// YONG
	__raw_writel(0, EXYNOS5410_BB_CON1);
