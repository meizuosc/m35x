...
int sec_clock_change_up(int level, int step)
{
	...
	if ((g_debug_CCB_Info_Flag % g_debug_CCB_count) == 0)
		pr_debug("SGX CCB RO : %d, WO : %d, Total : %d", *g_debug_CCB_Info_RO, *g_debug_CCB_Info_WO, g_debug_CCB_Info_WCNT);
...
void sec_gpu_dvfs_handler(int utilization_value)
{
	...
	if (utilization_value == 0) {
		...
		if (sgx_dvfs_level != (GPU_DVFS_MAX_LEVEL - 1)) {
			sgx_dvfs_level = GPU_DVFS_MAX_LEVEL - 1;
			sec_gpu_vol_clk_change(g_gpu_dvfs_data[sgx_dvfs_level].clock,
					g_gpu_dvfs_data[sgx_dvfs_level].voltage);
		}
		return;
	}
