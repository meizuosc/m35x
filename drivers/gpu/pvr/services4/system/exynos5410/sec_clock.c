...
void gpu_clock_set(int sgx_clk)
{
...
#ifdef DEBUG_BW
	{
...
#if defined(CONFIG_EXYNOS5410_BTS)
		{
			...
			pr_debug("SGX change clock [%d] Mhz -> [%d] MHz req [%d] MHz / M[%d] / B[%d]", old_clk, sgx_gpu_clk, sgx_clk, (800 / mif_sdiv), bts);
		}
#else
		pr_debug("SGX change clock [%d] Mhz -> [%d] MHz req [%d] MHz / M[%d]", old_clk, sgx_gpu_clk, sgx_clk, (800 / mif_sdiv));
#endif
	}
#endif
}
