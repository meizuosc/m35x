...
#ifdef CONFIG_CPU_FREQ_GOV_APRIL
extern void remote_hotplug_in(void);
#else
#define remote_hotplug_in()	do { } while (0)
#endif
...
static irqreturn_t gpio_keys_gpio_isr(int irq, void *dev_id)
{
	...
	if (bdata->timer_debounce) {
		pr_debug("%s: @@@@@@@@@@ prepare key report\n", __func__);
		if (bdata->timer_debounce > 1)
			remote_hotplug_in();
