...
#define FW_IMAGE_NAME "rmi4/M6X-TPK.img"
#define IMAGE_FIRMWARE_ID 1532497
...
#define LCD_SHARP_ID 1 
#define LCD_JDI_ID 0
#define LCD_DEFUALT_ID -1 
#define TPKW_FW_NAME "M65-TPKW"
#define TPKWTL2_FW_NAME "M65-TPKL2W"
...
static struct image_info m6x_images[] = {
	//{ .product_id = "s3202_ver5",.img_path = "rmi4/M6X-TPK.img", 1532497},
	{ .product_id = "M65-TPK",.img_path = "rmi4/M6X-TPK.img", 1533486,-1},/*T107 */
	{ .product_id = "M65-TPKB",.img_path = "rmi4/M6X-TPK.img", 1533486,-1},/*T107*/
	{ .product_id = "M65-TPKL2B",.img_path = "rmi4/M6X-TPKL2B.img", 1533486,-1},/*T107*/
	{ .product_id = "M65-TPKW",.img_path = "rmi4/M6X-TPKW.img", 1533486,LCD_SHARP_ID},/*T009*/
	{ .product_id = "M65-TPKL2W",.img_path = "rmi4/M6X-TPKL2W.img", 1533486,LCD_SHARP_ID},/*T019*/
	{ .product_id = "M65-TPKW",.img_path = "rmi4/M6X-JDITPKW.img", 1533486,LCD_JDI_ID},/*T018*/
	{ .product_id = "M65-TPKL2W",.img_path = "rmi4/M6X-JDITPKL2W.img", 1533486,LCD_JDI_ID},/*T028*/
	//{ .product_id = "M65-TPKHB",.img_path = "rmi4/M6X-TPKHB.img", 1533486,-1},
	//{ .product_id = "M65-AVC",.img_path = "rmi4/M6X-AVC.img", 0},
	//{ .product_id = "M65W-AVC",.img_path = "rmi4/M6X-AVC.img", 0},
};
...
static struct device_attribute attrs[] = {
	__ATTR(doreflash, RMI_WO_ATTR,
			synaptics_rmi4_show_error,
			fwu_sysfs_do_reflash_store),
	__ATTR(writeconfig, RMI_WO_ATTR,
			synaptics_rmi4_show_error,
			fwu_sysfs_write_config_store),
	__ATTR(readconfig, RMI_WO_ATTR,
			synaptics_rmi4_show_error,
			fwu_sysfs_read_config_store),
	__ATTR(configarea, RMI_WO_ATTR,
			synaptics_rmi4_show_error,
			fwu_sysfs_config_area_store),
	__ATTR(imagesize, RMI_WO_ATTR,
			synaptics_rmi4_show_error,
			fwu_sysfs_image_size_store),
	__ATTR(blocksize, RMI_RO_ATTR,
			fwu_sysfs_block_size_show,
			synaptics_rmi4_store_error),
	__ATTR(fwblockcount, RMI_RO_ATTR,
			fwu_sysfs_firmware_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(configblockcount, RMI_RO_ATTR,
			fwu_sysfs_configuration_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(permconfigblockcount, RMI_RO_ATTR,
			fwu_sysfs_perm_config_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(blconfigblockcount, RMI_RO_ATTR,
			fwu_sysfs_bl_config_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(dispconfigblockcount, RMI_RO_ATTR,
...
static int fwu_read_f34_flash_status(void)
{
	...
	if (retval < 0) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
				"%s: Failed to read flash status 0x%.2X\n",
...
static enum flash_area fwu_go_nogo(struct image_header *header)
{
	...
	if (retval < 0) {
		dev_info(&i2c_client->dev,
			"%s: fwu_read_f01_device_status < 0\n",
			__func__);
		flash_area = NONE;
		goto exit;
	}
	...
	if (header->is_contain_build_info) {
		...
		imageFirmwareID = IMAGE_FIRMWARE_ID;
		#if 0
		for(i= 0;i<(sizeof(m6x_images) / sizeof(m6x_images[0]));i++)
		{
			if( strcmp(m6x_images[i].product_id,fwu->product_id) == 0)
			{
				imageFirmwareID = m6x_images[i].firmware_id;
				break;
			}
		}
		#else 
		 if(fwu->fw_id != -1)
		 	imageFirmwareID = m6x_images[fwu->fw_id].firmware_id;
		#endif
	}
	
	dev_info(&i2c_client->dev,
			"Device firmware id %d, Image firmware id %d\n",
			deviceFirmwareID,
			imageFirmwareID);
	...
	deviceConfigID = simple_strtoul(&config_id[1],NULL,10);

	dev_info(&i2c_client->dev,
		"Device config ID %c%c%c%c(%d)\n",
		config_id[0], config_id[1], config_id[2], config_id[3],deviceConfigID);

	/* .img config id */
	//imageConfigID =  extract_uint(fwu->config_data);
	imageConfigID = (int)simple_strtoul(&fwu->config_data[1],NULL,10);
	
	dev_info(&i2c_client->dev,
			"Image config ID %c%c%c%c(%d)\n",
			fwu->config_data[0],
			fwu->config_data[1],
			fwu->config_data[2],
			fwu->config_data[3],
...
static int fwu_start_reflash(void)
{
	...
	struct image_header header;
	const unsigned char *fw_image;
	const unsigned char *fw_image_path = NULL;
	const struct firmware *fw_entry = NULL;
	struct f01_device_status f01_device_status;
	enum flash_area flash_area;

	pr_info("%s: Start of reflash process------------------------------------------------\n", __func__);
	pr_info("%s: product id %s\n", __func__,fwu->product_id);


	fwu->fw_id =  -1 ;
	
	//fw_image_path = FW_IMAGE_NAME;
	for(i= 0;i<(sizeof(m6x_images) / sizeof(m6x_images[0]));i++)
	{
		 if( strcmp(m6x_images[i].product_id,fwu->product_id) == 0){
			
			if( m6x_images[i].img_path == NULL ){
				snprintf(firmware_name, sizeof(firmware_name), "rmi4/%s.img",
					fwu->product_id);
				fw_image_path = firmware_name;
			}else{
				  if(!strcmp(fwu->product_id,TPKW_FW_NAME)\
				  	||!strcmp(fwu->product_id,TPKWTL2_FW_NAME)){
						if(fwu->lcd_id==m6x_images[i].lcd_id){
						    fw_image_path = m6x_images[i].img_path;
							printk("RMI->%s:firmware name[%s]\n",__func__,fw_image_path);
	...
	if (fwu->ext_data_source)
		...
		retval = request_firmware(&fw_entry, fw_image_path,
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	fwu->lcd_id = __lcd_read_id();
	if(fwu->lcd_id== LCD_SHARP_ID){
		printk("rmi->%s:SHARP LCD\n",__func__);
	}else
		printk("rmi->%s:JDI LCD\n",__func__);
