...
struct synaptics_rmi4_data {
	...
	struct synaptics_rmi4_fn * f11_handle;
	int irq;
	int sensor_max_x;
	int sensor_max_y;
	bool touch_stopped;
	...
	bool flip_y;
	bool touch_debug;
	bool touch_partitioned;
	bool touch_swipe;
	bool touch_f11maskreport;
	bool touch_unlock_key;
	bool touch_clear_keystatus;
	struct notifier_block notifier;
	struct synaptics_rmi4_f1a_handle *f1a_handle;
...
#define concat(a, b,c) tricat_(a, b,c)
...
static ssize_t concat(synaptics_rmi4_f,FNUM,  _##propname##_show)(\
		struct device *dev,\
		struct device_attribute *attr,\
		char *buf);\
\
struct device_attribute dev_attr_##propname =\
		__ATTR(propname, RMI_RO_ATTR,\
		concat(synaptics_rmi4_f,FNUM, _##propname##_show),\
		synaptics_rmi4_store_error);

#define store_prototype(propname)\
static ssize_t concat(synaptics_rmi4_f,FNUM, _##propname##_store)(\
		struct device *dev,\
		struct device_attribute *attr,\
		const char *buf, size_t count);\
\
struct device_attribute dev_attr_##propname =\
		__ATTR(propname, RMI_WO_ATTR,\
		synaptics_rmi4_show_error,\
		concat(synaptics_rmi4_f,FNUM, _##propname##_store));
...
static ssize_t concat(synaptics_rmi4_f,FNUM, _##propname##_show)(\
		...
		char *buf)\
{\
	struct FUNCTION_DATA *data = concat(f,FNUM,);\
	return snprintf(buf, PAGE_SIZE, fmt, data->rtype.propname);\
} \
...
{\
	int retval;\
	struct FUNCTION_DATA *data = concat(f,FNUM,);\
	struct synaptics_rmi4_data *rmi4_data = data->rmi4_data;\
\
	mutex_lock(&data->rtype##_mutex);\
\
	retval = data->fn_ptr->read(rmi4_data,\
			data->rtype.rgrp->address,\
			data->rtype.rgrp->data,\
			sizeof(data->rtype.rgrp->data));\
	mutex_unlock(&data->rtype##_mutex);\
	if (retval < 0) {\
...
\
	if (data->rtype.rgrp->propname == setting) {\
		mutex_unlock(&data->rtype##_mutex);\
		return count;\
	} \
\
	o_setting = data->rtype.rgrp->propname;\
	data->rtype.rgrp->propname = setting;\
\
	retval = data->fn_ptr->write(rmi4_data,\
			data->rtype.rgrp->address,\
			data->rtype.rgrp->data,\
			sizeof(data->rtype.rgrp->data));\
	if (retval < 0) {\
		dev_err(&rmi4_data->i2c_client->dev,\
				...
				" " #rgrp "\n",\
				__func__);\
		data->rtype.rgrp->propname = o_setting;\
		mutex_unlock(&data->rtype##_mutex);\
		return retval;\
	} \
\
...
\
	mutex_lock(&data->rtype##_mutex);\
\
	length = data->rtype.rgrp->length;\
\
	retval = data->fn_ptr->read(rmi4_data,\
			data->rtype.rgrp->address,\
			(unsigned char *)data->rtype.rgrp->data,\
			length);\
	mutex_unlock(&data->rtype##_mutex);\
	if (retval < 0) {\
...
\
	for (ii = 0; ii < length; ii++) {\
		retval = snprintf(temp, PAGE_SIZE - size, fmt " ",\
				data->rtype.rgrp->data[ii].propname);\
		if (retval < 0) {\
			dev_err(&rmi4_data->i2c_client->dev,\
...
\
	for (ii = 0; ii < length; ii++) {\
		if (sscanf(temp, fmt, &setting) == 1) {\
			data->rtype.rgrp->data[ii].propname = setting;\
		} else {\
			retval = data->fn_ptr->read(rmi4_data,\
					data->rtype.rgrp->address,\
					(unsigned char *)data->rtype.rgrp->data,\
