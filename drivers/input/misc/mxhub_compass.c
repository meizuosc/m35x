	...
	int length)
{
	...
	reg +=MX_HUB_REG_COMPASS_BASE_S;	

	ret = mx_hub_readdata_rdy(i2c,reg,length,rxData);
	...
	int length)
{
	int ret;
#if 1
	txData[0] +=MX_HUB_REG_COMPASS_BASE_S;

	ret = mx_hub_writedata_rdy(i2c,0xFF ,length,txData);
...
{
	pr_info("akm_enable_acc_store from sysfs: %s", buf);
...
static ssize_t akm_enable_mag_store(
	...
	char const *buf, size_t count)
{
	pr_info("akm_enable_mag_store from sysfs: %s", buf);
...
static ssize_t akm_enable_fusion_store(
	...
	char const *buf, size_t count)
{
	pr_info("akm_enable_fusion_store from sysfs: %s", buf);
...
static ssize_t akm_delay_acc_store(
	...
	char const *buf, size_t count)
{
	pr_info("akm_delay_acc from sysfs: %s", buf);
...
static ssize_t akm_delay_mag_store(
	...
	char const *buf, size_t count)
{
	pr_info("akm_delay_mag from sysfs: %s", buf);
...
static ssize_t akm_delay_fusion_store(
	...
	char const *buf, size_t count)
{
	pr_info("akm_delay_fusion from sysfs: %s", buf);
...
#endif
/*self test begin*/
static int akm_self_test(struct akm_compass_data *s_akm, int *xyz)
{
	int err = 0,cnt;
	...
	buf[1] = 0x40;
	akm_i2c_txdata(s_akm->i2c, buf, 2);
	msleep(10);
	...
	while(cnt--){
		...
		err = akm_i2c_rxdata(s_akm->i2c, sbuf,1);
		if(err < 0)
			return err;
		pr_debug("%s: sbuf[0] = 0x%X (count = %d)\n",__func__, sbuf[0],cnt);
		if(sbuf[0] == 1)
			break;			
	}

	s_data[0] = AK8963_REG_HXL;
	err = akm_i2c_rxdata(s_akm->i2c, s_data,sizeof(s_data));	

	//i2c_smbus_read_i2c_block_data(s_akm->i2c, AK8963_REG_HXL,
	...
	/* Hadj = (H*(Asa+128))/256 */
	x = (x*(s_akm->sense_conf[0] + 128)) >> 8;
	y = (y*(s_akm->sense_conf[1] + 128)) >> 8;
	z = (z*(s_akm->sense_conf[2] + 128)) >> 8;
...
static ssize_t akm_self_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0;
	int xyz[3];
	struct akm_compass_data *s_akm = dev_get_drvdata(dev);

	ret = akm_self_test(s_akm, xyz);
...
static struct device_attribute akm_compass_attributes[] = {
	__ATTR(akm8963_self_test, 0660, akm_self_show, NULL),
...
static int akm8963_i2c_check_device(struct akm_compass_data* akm)
{
	/* AK8963 specific function */
	...
	dev_info(&client->dev,"%s: id = 0x%X.", __func__,akm->sense_info[0]);
	if (err < 0)
		return err;
	...
	while(i--)
	{
		...
		if (err < 0)
			return err;
		if( (akm->sense_conf[0] != 0x00 && akm->sense_conf[0] != 0xFF)
			&&(akm->sense_conf[1] != 0x00 && akm->sense_conf[1] != 0xFF)
			&&(akm->sense_conf[2] != 0x00 && akm->sense_conf[2] != 0xFF)
			)
		break;
	}
	
	dev_info(&client->dev,"%s: sense_conf = 0x%X 0x%X 0x%X .", __func__,akm->sense_conf[0],akm->sense_conf[1],akm->sense_conf[2]);
...
static int __devinit akm_compass_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "start probing.");
	...
	if (pdata) {
		...
		s_akm->layout = pdata->compass_layout;
		s_akm->gpio_rstn = 0;//pdata->gpio_RSTN;
	} else {
		/* Platform data is not available.
		   Layout and information should be set by each application. */
		dev_dbg(&pdev->dev, "%s: No platform data.", __func__);
		s_akm->layout = 0;
		s_akm->gpio_rstn = 0;
	}
	...
	for (i = 0; i < 5; i++)
	{
		err = akm8963_i2c_check_device(s_akm);
		if (err < 0)
			msleep(100);
		else
			break;
	...
	/***** IRQ setup *****/
	if(AKM_IRQ_ENABLE)
		s_akm->irq = pdata->irq_base + MX_HUB_IRQ_COMPASS;
	
	if (s_akm->irq) {
		err = request_threaded_irq(s_akm->irq, 0, akm_compass_irq,
			0, "mx-hub-compass"/*pdev->name*/, s_akm);
		if (unlikely(err < 0)) {
			dev_err(&pdev->dev, "mx hub: failed to request compass IRQ %d\n",
				s_akm->irq);
			goto exit4;
		}
	}
	
	dev_dbg(&pdev->dev, "%s: IRQ is #%d.",
			__func__, s_akm->irq);
	...
	dev_set_drvdata(&pdev->dev,s_akm);
...
exit2:
	kfree(s_akm);
exit1:
	dev_info(&pdev->dev, "fail probed.");
	return err;
}

static int akm_compass_remove(struct platform_device *pdev)
{
	struct akm_compass_data *akm = platform_get_drvdata(pdev);

	remove_sysfs_interfaces(akm);
	if (misc_deregister(&akm_compass_dev) < 0)
		dev_err(&pdev->dev, "misc deregister failed.");
	if (akm->irq)
...
const struct platform_device_id akm_compass_id[] = {
	{ "mx-hub-compass", 0 },
	{ },
};
...
static struct platform_driver akm_compass_driver = {
	.driver = {
		.name  = "mx-hub-compass",
		.owner = THIS_MODULE,
	...
	.remove = __devexit_p(akm_compass_remove),
	.id_table = akm_compass_id,
};

static int __init akm_compass_init(void)
{
	pr_debug("AKM compass driver: initialize.");
	return platform_driver_register(&akm_compass_driver);
}

static void __exit akm_compass_exit(void)
{
	pr_debug("AKM compass driver: release.");
	platform_driver_unregister(&akm_compass_driver);
}
