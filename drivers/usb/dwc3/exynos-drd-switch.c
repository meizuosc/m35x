...
static int exynos_drd_switch_start_peripheral(struct usb_otg *otg, int on)
{
	...
	if (on) {
		...
		if (!exynos_drd_switch_is_host_off(otg)) {
			 ...
			 */
			dev_dbg(otg->phy->dev, "%s: host is still active\n",
						__func__);
			return -EAGAIN;
		}
...
static void exynos_drd_switch_work(struct work_struct *w)
{
	...
	case OTG_STATE_A_IDLE:
		if (id_state == B_DEV) {
			...
			if (!ret || ret == -EINPROGRESS) {
				...
				dev_dbg(phy->dev, "host turn on retry\n");
				exynos_drd_switch_schedule_dwork(work,
								 EAGAIN_DELAY);
			} else {
				/* Fatal error */
	...
	case OTG_STATE_A_HOST:
		...
		if (id_state == B_DEV) {
			...
			if (ret == -EAGAIN) {
				dev_dbg(phy->dev, "host turn off retry\n");
				exynos_drd_switch_schedule_dwork(work,
								 EAGAIN_DELAY);
			} else {
...
void exynos_drd_switch_reset(struct exynos_drd *drd, int run)
{
	...
	if (otg) {
		...
		printk("[usb] %s, vbus: %d, id: %d\n", __func__, 
			drd_switch->vbus_active, drd_switch->id_state); 
	}
}
...
int exynos_drd_switch_init(struct exynos_drd *drd)
{
	...
	if (drd_switch->id_irq >= 0) {
		ret = devm_request_threaded_irq(drd->dev, drd_switch->id_irq,
				  NULL, exynos_drd_switch_id_interrupt, irq_flags,
	...
	if (drd_switch->vbus_irq >= 0) {
		ret = devm_request_threaded_irq(drd->dev, drd_switch->vbus_irq,
				  NULL, exynos_drd_switch_vbus_interrupt, irq_flags,
	...
	// enable wakable irq
	printk("%s, setting vbus irq: %d\n", __func__,  drd_switch->vbus_irq); 
	if (enable_irq_wake(drd_switch->vbus_irq)) {	// fail
		dev_err(drd->dev, "cannot get wakable VBUS irq:%d\n", drd_switch->vbus_irq);
		goto err_irq;
	}
