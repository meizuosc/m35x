...
#define MTP_CPU_QOS_FREQ_LOW		(600000)
#define MTP_CPU_QOS_FREQ_HIGH		(1200000)
#define MTP_MIF_QOS_FREQ_LOW		(400000)
#define MTP_MIF_QOS_FREQ_HIGH		(800000)
	...
	size_t count, loff_t *pos)
{
	...
	while (count > 0 || sendZLP) {
		...
		if (dev->state != STATE_BUSY) {
			ERROR(cdev, "mtp_write dev->error\n");
			r = -EIO;
			break;
		}
		...
		if (ret < 0) {
			ERROR(cdev, "mtp_write: xfer error %d\n", ret);
			r = -EIO;
			break;
		}
...
static void send_file_work(struct work_struct *data)
{
	...
	while (count > 0 || sendZLP) {
		...
		ret = usb_ep_queue(dev->ep_in, req, GFP_KERNEL);
		if (ret < 0) {
			ERROR(cdev, "send_file_work: xfer error %d\n", ret);
			dev->state = STATE_ERROR;
			r = -EIO;
			break;
		}
				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	if (ctrl->bRequestType ==
		...
		if (ctrl->bRequest == MTP_REQ_CANCEL && w_index == 0
			...
			INFO(cdev, "MTP_REQ_GET_DEVICE_STATUS\n");
			spin_lock_irqsave(&dev->lock, flags);
...
static int mtp_setup(void)
{
	...
	pm_qos_add_request(&mtp_cpu_qos, PM_QOS_CPU_FREQ_MIN, 0);
