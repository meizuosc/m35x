 ...
 * @gfp_flags: the type of bounce buffer memory to allocate.
 *
				 ...
				 gfp_t gfp_flags)
{
	...
	if (!udc_ep->dir_in) {
		...
		if (rem > 0) {
			dev_dbg(udc->dev, "Buf length isn't multiple of max packet size\n");
				   ...
				   struct exynos_ss_udc_ep *udc_ep)
{
	...
	/*
	 * Do not end ep0's transfer, it will cause the USB
	 * device controller trap into unrecoverable state
	 * and doesn't work. (This may be the hardware's bug)
	 * I portting from driver/usb/dw3/gadget.c.
	 */
	...
	if (res < 0) {
		dev_err(udc->dev, "Failed to end transfer, ep%d %s, tri=%d\n",
			    udc_ep->epnum, udc_ep->dir_in ? "in" : "out", udc_ep->tri);
		udc_ep->not_ready = 1;
	}
...
//	printk("%s: %s, req %p\n", __func__, udc_ep->name, ureq); // hunsoo.lee
	...
	if (udc_ep->halted) {
		dev_dbg(udc->dev, "%s: ep is stalled\n", __func__);
		return;
	}
	...
	if (epnum == 0)
		...
		case EP0_WAIT_NRDY:
			dev_dbg(udc->dev, "Wait XferNotReady for EP0\n");
			return;
...
#if 0
	printk( "complete: ep %p %s, req %p, %d => %p\n",
				   ...
				   const struct usb_endpoint_descriptor *desc)
{
	...
	dev_dbg(udc->dev, "%s: %s: a 0x%02x, attr 0x%02x, mps 0x%04x, intr %d\n",
	...
	case USB_ENDPOINT_XFER_ISOC:
		dev_dbg(udc->dev, "Isochronous endpoint\n");
				  ...
				  gfp_t gfp_flags)
{
	...
	dev_dbg(udc->dev, "%s: %s (%p): %d@%p, noi=%d, zero=%d, snok=%d\n",
	...
	if (req->buf == udc->ctrl_buff)
		...
		ret = exynos_ss_udc_map_dma(udc, udc_req, udc_ep, gfp_flags);
		if (ret)
			return ret;
	}
					  ...
					  struct usb_ctrlrequest *ctrl)
{
	...
	if (ret == 0 && udc->driver) {
		...
		dev_dbg(udc->dev, "ctrl: pass request to gadget\n");
	...
	if (ret < 0) {
		dev_dbg(udc->dev, "ctrl: ep0%s stall (err %d)\n",
					  ...
					  u32 event)
{
	...
	if (cmdtyp == EXYNOS_USB3_DEPCMDx_CmdTyp_DEPENDXFER) {
		...
		list_for_each_entry_safe(epcmd, tepcmd,
			...
			dev_dbg(udc->dev, "Pending command %02xh\n",
					    epcmd->cmdtyp);
					...
					u32 event)
{
	...
	if (udc_ep->dir_in) {
		...
		if (size_left) {
			...
			if (trb_status == EXYNOS_USB3_TRB_TRBSTS_MissedIsoc)
				 ...
				 * in this case? */
				dev_dbg(udc->dev, "Isoc interval missed\n");
			else
				/* REVISIT shall we -ECONNRESET here? */
				dev_dbg(udc->dev, "Incomplete In xfer\n");
		}
	...
	if (udc_ep->dir_in && req->zero) {
		if (req->length && req->length == req->actual &&
			...
			dev_dbg(udc->dev, "Send ZLP to complete transfer\n");
	...
	if (udc_ep->epnum == 0) {
		...
		case EP0_SETUP_PHASE:
			...
			if (status !=
				...
				dev_err(udc->dev, "Unexpected XferNotReady "
...
static void exynos_ss_udc_handle_depevt(struct exynos_ss_udc *udc, u32 event)
{
	...
	case EXYNOS_USB3_DEPEVT_EVENT_XferInProgress:
		dev_dbg(udc->dev, "Xfer In Progress\n");
...
static irqreturn_t exynos_ss_udc_irq(int irq, void *pw)
{
	...
	dev_dbg(udc->dev, "IRQ: %d event(s)\n", gevntcount >> 2);

	while (gevntcount) {
		event = udc->event_buff[indx++];

		dev_dbg(udc->dev, "event[%p] = 0x%08x\n",
