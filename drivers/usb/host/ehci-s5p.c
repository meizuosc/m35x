...
#ifdef CONFIG_HAS_WAKELOCK
static struct wake_lock s5p_pm_lock;
...
static void s5p_ehci_clk_en(struct s5p_ehci_hcd *s5p_ehci)
{
	if (!atomic_read(&s5p_ehci->clk_cnt)) {
		clk_enable(s5p_ehci->clk);
		atomic_inc_return(&s5p_ehci->clk_cnt);
	}
}

static void s5p_ehci_clk_dis(struct s5p_ehci_hcd *s5p_ehci)
{
	if (atomic_read(&s5p_ehci->clk_cnt)) {
		clk_disable(s5p_ehci->clk);
		atomic_dec_return(&s5p_ehci->clk_cnt);
	}
}

int s5p_ehci_check_op(struct platform_device *pdev)
{
	struct s5p_ehci_hcd *s5p_ehci = platform_get_drvdata(pdev);
	struct s5p_ehci_platdata *pdata = pdev->dev.platform_data;

	if (pdata && pdata->phy_check_op) {
		if (pdata->phy_check_op(pdev, S5P_USB_PHY_HOST))
			return 1;
	}
	modem_set_active_state(0);
	s5p_ehci_clk_dis(s5p_ehci);

	return 0;
}
EXPORT_SYMBOL_GPL(s5p_ehci_check_op);

#define RETRY_CNT_LIMIT	30
int s5p_wait_for_cp_resume(struct platform_device *pdev, int port)
{
	struct s5p_ehci_hcd *s5p_ehci = platform_get_drvdata(pdev);
	struct usb_hcd *hcd = s5p_ehci->hcd;
	struct ehci_hcd *ehci = hcd_to_ehci(hcd);
...
EXPORT_SYMBOL_GPL(s5p_wait_for_cp_resume);
...
static int s5p_ehci_runtime_resume(struct device *dev)
{
	...
	dev_dbg(dev, "%s init_resume %d\n", __func__, init_resume);
	if (dev->power.is_suspended)
		return 0;
	s5p_wake_lock();

	s5p_ehci_clk_en(s5p_ehci);

	/* first resume back from Sleep is deferent ??? 
	 *  TODO: need lots of suspend/resume test
	 */
	if (init_resume)
		return s5p_ehci_init_resume(dev);

	/* make sure phy active for resume/runtime-resume hcd */
...
void s5p_ehci_power(struct platform_device *pdev, int power_on)
{
	struct s5p_ehci_hcd *s5p_ehci = platform_get_drvdata(pdev);
	struct usb_hcd *hcd = s5p_ehci->hcd;
	int retval;
	...
	if (power_on == 0 && s5p_ehci->power_on) {
		...
		if (retval < 0) {
			dev_err(&pdev->dev, "Power On Fail\n");
			return;
		}
		s5p_ehci->power_on = 1;
		...
		/* mif allow the ehci runtime after anumeration */
		pm_runtime_forbid(&pdev->dev);
		modem_set_active_state(1);
	}
}
EXPORT_SYMBOL_GPL(s5p_ehci_power);
