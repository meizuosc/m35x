...
static struct pm_qos_request softap_mif_qos;
...
dhd_get_stats(struct net_device *net)
{
	...
	if (dhd->pub.up) {
		if (module_remove) {
			printf("%s: module removed. ifp=%p, dhd=%p\n",
							__func__, ifp, dhd);
		} else
...
dhd_stop(struct net_device *net)
{
	...
	if (pm_qos_request_active(&softap_mif_qos))
		pm_qos_remove_request(&softap_mif_qos);
exit:
...
dhd_open(struct net_device *net)
{
	...
	if (strstr(fw_path, "fw_bcmdhd_apsta"))
		pm_qos_add_request(&softap_mif_qos, PM_QOS_BUS_THROUGHPUT, 200000);
exit:
	if (ret)
		dhd_stop(net);
