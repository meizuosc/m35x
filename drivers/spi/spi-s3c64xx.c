						...
						struct spi_device *spi)
{
	...
	if (!strncmp(spi->modalias, "ms2r_spi", sizeof(spi->modalias))) {
		/*
		* This is needed by ISP of MS2R.
		*/
		udelay(80);
	}
}
					    ...
					    struct spi_message *msg)
{
	...
	if (sdd->cur_speed != spi->max_speed_hz
		...
		sdd->cur_mode = spi->mode;
		sdd->data_not_swap = spi->data_not_swap;
...
static void s3c64xx_spi_hwinit(struct s3c64xx_spi_driver_data *sdd, int channel)
{
	...
	val = readl(regs + S3C64XX_SPI_MODE_CFG);
	if (sci->dma_type == SPI_DMA_1_BURST) {
...
static int s3c64xx_spi_resume(struct device *dev)
{
	...
	/* s3c64xx_spi0_cfg_gpio() cause 8.5 mA power leak, so resume need config spi PULL_NONE for save power */
	if (pdev->id == 0)
		s3c_gpio_cfgall_range(MEIZU_ISP_SPI_CLK, 4, S3C_GPIO_INPUT, S3C_GPIO_PULL_NONE);	
