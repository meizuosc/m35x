...
struct pm_qos_request *g_exynos5_mif_qos=0;
struct pm_qos_request *g_exynos5_int_qos=0;
EXPORT_SYMBOL(g_exynos5_mif_qos);
EXPORT_SYMBOL(g_exynos5_int_qos);
static unsigned int last_int_qos;
static unsigned int last_mif_qos;
#endif
static unsigned int bandwidth;
unsigned int clk_down_request=0;
EXPORT_SYMBOL(clk_down_request);
unsigned int clk_gating_request=0;
EXPORT_SYMBOL(clk_gating_request);
bool *clk_enabled=0;
EXPORT_SYMBOL(clk_enabled);
struct clk * clk_lcd=0;
EXPORT_SYMBOL(clk_lcd);
struct clk * clk_axi_disp1=0;
EXPORT_SYMBOL(clk_axi_disp1);
struct clk * clk_fimd=0;
EXPORT_SYMBOL(clk_fimd);

#ifdef DYNAMIC_VIDEO_FPS_CONTROL // need to debug dynamic control both for command mode and video mode

#include <plat/regs-mipidsim.h>

#include <linux/sched.h>
extern unsigned int mipi_framedone_status;

extern u64 interrupt_timestamp[4];

#endif

u32 gpio_gic_base=0;
EXPORT_SYMBOL(gpio_gic_base);
...
#define VSYNC_TIMEOUT_MSEC 50 // for 60Hz 16.6ms*3frame~=50ms, 30Hz 100ms
...
#ifdef CONFIG_FB_LCD_WQXGA /* For WQXGA */
...
#define FIMD_INT_BUS_MIN    0
#define FIMD_INT_BUS_LOW    160000 /* Temporary */
#define FIMD_INT_BUS_MID    200000
#define FIMD_INT_BUS_HIG    267000
#define FIMD_INT_BUS_MAX    400000
#endif
...
#if defined(CONFIG_ARM_EXYNOS5410_BUS_DEVFREQ)
extern unsigned int g_miffreq; // Originally extern variable // TBD
#endif

/* Parameters of Dynamic Frame Rate Control for Video Mode */
#ifdef CONFIG_S3C_FPS_ADJUSTING
#define FRAMEDOWN_THRESHOLD 30 // TRANSITION COUNT for FRAME, 60, 20 checked
#define FPS_60HZ 60
#define FPS_40HZ 40
int s3c_fb_get_cur_win_buf_addr(struct fb_info *info, int id);
#endif

#define PSR_THRESHOLD 15 // TRANSITION COUNT for Getting in PSR
...
struct s3c_fb_psr {
	wait_queue_head_t	wait;
	struct mutex    lock;        
	unsigned int frame_noupdate_count;
	bool        goPSR;
	bool    inStateOfChangeToPSR;
	bool    inStateOfPSR;
	struct task_struct	*thread;
};
 ...
 * @fps_lock: The spinlock protection for fps locking data sturcture.
 ...
 * @irq_te_no: IRQ line number for TE interrupt
...
struct s3c_fb {
	...
	int			irq_no;
	int    			s3c_fb_update_request;
	int			irq_te_no;
	bool		 	clk_gating;
	bool        		clk_down;
	bool		 	clk_enabled;
	bool        		te_irq_disabled;
	bool        		sysmmu_disabled;
	struct s3c_fb_vsync	vsync_info;
	struct s3c_fb_vsync  	te_info;
	struct s3c_fb_psr 	psr_info;
	bool			power_state;
...
static struct s3c_fb *g_sfb = NULL;
//------- Function Definition
static int s3c_fb_wait_for_vsync(struct s3c_fb *sfb, u32 timeout);
static int s3c_fb_wait_for_te(struct s3c_fb *sfb, u32 timeout);
static void s3c_fb_psr_enable(struct s3c_fb *sfb);
static void s3c_fb_psr_disable(struct s3c_fb *sfb);
...
#ifdef CONFIG_S3C_FPS_ADJUSTING
static void s3c_fb_enable_vfp_irq(struct s3c_fb *sfb)
{
...
static void s3c_fb_enable_vsync_irq(struct s3c_fb *sfb)
{
	void __iomem *regs = sfb->regs;
	u32 irq_ctrl_reg;
...
static void s3c_fb_fps_adjust_to_40_in_vfp(struct s3c_fb *sfb)
{
	u32 fimd1_ratio;
	unsigned long flags;

	spin_lock_irqsave(&sfb->fps_lock, flags);

	fimd1_ratio = readl(sfb->clk_div_disp10) & ~(0xf);

	writel((fimd1_ratio | 0x5), sfb->clk_div_disp10);

	spin_unlock_irqrestore(&sfb->fps_lock, flags);
}

static void s3c_fb_fps_adjust_to_60_in_vfp(struct s3c_fb *sfb)
{
	u32 fimd1_ratio;
	unsigned long flags;
	...
	writel((fimd1_ratio | 0x3), sfb->clk_div_disp10);

	spin_unlock_irqrestore(&sfb->fps_lock, flags);
}

static ssize_t show_framerate(struct device *device, struct device_attribute *attr, char *buf)
{
	struct s3c_fb *sfb = dev_get_drvdata(device);
	return sprintf(buf, "%d\n", sfb->current_fps);
}

static ssize_t store_framerate(struct device *device,
		...
		const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "40", 2)) {
		sfb->target_fps = FPS_40HZ;
		s3c_fb_enable_vfp_irq(sfb);
	} else if (!strncmp(buf, "60", 2)) {
		sfb->target_fps = FPS_60HZ;
		s3c_fb_enable_vfp_irq(sfb);
	} else {
		dev_err(sfb->dev, "Invalid framerate\n");
	}

	return count;
}

static DEVICE_ATTR(framerate, S_IRWXUGO, show_framerate, store_framerate);
#endif
 ...
 */
static int __maybe_unused s3c_fb_get_enabled_num_of_win(struct s3c_fb *sfb)
{
...
static void s3c_fb_dump_registers(struct s3c_fb *sfb)
{
	...
	pr_err("...\n");
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
			sfb->regs + 0x20000, 0x200, false);
#endif
#if 0 //Debugging
	pr_info("EXYNOS5_CLKDIV_TOP2 : 0x%0x = 0x%0x\n", EXYNOS5_CLKDIV_TOP2, readl(EXYNOS5_CLKDIV_TOP2));			// dout_aclk_300_disp1	
	pr_info("EXYNOS5_CLKSRC_TOP3 : 0x%0x = 0x%0x\n", EXYNOS5_CLKSRC_TOP3, readl(EXYNOS5_CLKSRC_TOP3));				
	pr_info("EXYNOS5_CLKSRC_DISP1_0 : 0x%0x = 0x%0x\n", EXYNOS5_CLKSRC_DISP1_0, readl(EXYNOS5_CLKSRC_DISP1_0));				
	pr_info("EXYNOS5_CLKGATE_IP_DISP1 : 0x%0x = 0x%0x\n", EXYNOS5_CLKGATE_IP_DISP1, readl(EXYNOS5_CLKGATE_IP_DISP1));			// fimd1==lcd, smmufimd1, dsim1, mixer, mie1,...	
	pr_info("EXYNOS5_CLKGATE_BUS_DISP1 : 0x%0x = 0x%0x\n", EXYNOS5_CLKGATE_BUS_DISP1, readl(EXYNOS5_CLKGATE_BUS_DISP1));			// axi_disp1
    pr_info("CLK_GATE_TOP_SCLK_DISP1        : 0x%08x\n", __raw_readl(EXYNOS5_CLKGATE_TOP_SCLK_DISP1));
	pr_info("EXYNOS5_CLKSRC_MASK_DISP1_0 : 0x%0x = 0x%0x\n", EXYNOS5_CLKSRC_MASK_DISP1_0, readl(EXYNOS5_CLKSRC_MASK_DISP1_0));				
	pr_info("DISP1_OPTION                   : 0x%08x\n", __raw_readl(EXYNOS5410_DISP1_OPTION));
	pr_info("DISP1_CONFIG : 0x%0x = 0x%0x\n", EXYNOS5410_DISP1_CONFIGURATION, readl(EXYNOS5410_DISP1_CONFIGURATION));				
			    ...
			    struct fb_info *info)
{
	...
	dev_dbg(sfb->dev, "%s: x:%d, y:%d\n", __func__, x, y);

	hz = 1000000000000ULL;		/* 1e12 picoseconds per second */

	hz += (x * y) / 2;
	do_div(hz, x * y);		/* divide by x * y with rounding */
	dev_dbg(sfb->dev, "%s: hz/(x*y)=%llu/pixclock:%d\n", __func__, hz, var->pixclock);

	hz += var->pixclock / 2;
	do_div(hz, var->pixclock);	/* divide by pixclock with rounding */

	win->fps = hz;
	dev_dbg(sfb->dev, "%s: hz:%llu, pixclock:%d, win->fps:%d\n", 
                __func__, hz, var->pixclock, win->fps);
...
static int s3c_fb_calc_pixclk(struct s3c_fb *sfb, unsigned int pixclk)
{
	...
	result = ((unsigned int)(tmp)*11/10) / 1000; // Increase small frame timing to use lower clock
	dev_dbg(sfb->dev, "lcd_clk:%c srcclk:%ld(hz) * pixclk:%d(ns) / 1G /1000 = div:%d\n",
		(sfb->variant.has_clksel) ? 'n' : 'y', clk, pixclk, result);
...
static inline u32 wincon(u32 bits_per_pixel, u32 transp_length, u32 red_length)
{
	...
	case 32:
		...
		// This is related with VIDCON2_WB_ENABLE; 
		// and Module C2h command and porch difference
		data |= WINCONx_WSWP; 
		data |= WINCONx_BURSTLEN_16WORD;
		break;
	}
...
static int s3c_fb_set_par(struct fb_info *info)
{
	...
	if (!s3c_fb_en_state(sfb))
		dev_warn(sfb->dev, "%s : FB P/C ready check @ %d.\n", __func__, __LINE__);
...
static int s3c_fb_blank(int blank_mode, struct fb_info *info)
{
	...
	case FB_BLANK_NORMAL:
		/*here we should give the control rights to mipi dsi.
		 because it needs the fimd running when it is suspending.
		 please refer to unblank notify of s5p_mipi_dsi.c
		 */		 
...
static void s3c_fb_enable_irq(struct s3c_fb *sfb)
{
	...
	while (irq_ctrl_reg != readl(regs + VIDINTCON0) && count--) {
		mdelay(10);
		writel(irq_ctrl_reg, regs + VIDINTCON0);
		pr_info("%s failed to write FIMD int controller, try again!\n", __func__);
	}
...
void s3c_fb_psr_trigger_set(struct s3c_fb *sfb, int mode)
{
	unsigned int data;
	...
	if (mode) {
		data &= ~(HWTRIGEN_PER_RGB |HWTRGMASK_I80_RGB | HWTRGEN_I80_RGB
				| SWTRGCMD_I80_RGB | TRGMODE_I80_RGB);
		data |= HWTRIGEN_PER_RGB| HWTRGMASK_I80_RGB | HWTRGEN_I80_RGB;
	}
	else {
		data &= ~(HWTRIGEN_PER_RGB |HWTRGMASK_I80_RGB | HWTRGEN_I80_RGB
...
void s3c_fb_enable_display_clock(struct s3c_fb *sfb)
{
	u32 clksrc_top3, clksrc_disp10; 

	if (sfb->clk_down) {
		// we need to adjust voltage here directly
		//pm_qos_update_request(&exynos5_int_qos, 100000);

		clksrc_top3 = readl(EXYNOS5_CLKSRC_TOP3);
		clksrc_top3 |= (1<<19);
		writel(clksrc_top3, EXYNOS5_CLKSRC_TOP3);
		clksrc_top3 = readl(EXYNOS5_CLKSRC_TOP3);
		//FIMD1_SEL
		clksrc_disp10 = readl(EXYNOS5_CLKSRC_DISP1_0);
#if defined(CONFIG_FB_VIDEO_PSR)
		clksrc_disp10 |= (6<<0); // MPLL_BPLL
#endif
		writel(clksrc_disp10, EXYNOS5_CLKSRC_DISP1_0);
		clksrc_disp10 = readl(EXYNOS5_CLKSRC_DISP1_0);
	}

	mutex_lock(&sfb->clock_lock);
	if (sfb->clk_gating && !sfb->clk_enabled) {
		...
		if (sfb->sysmmu_disabled){
			iovmm_activate(&s5p_device_fimd1.dev);
			sfb->sysmmu_disabled=false;
		}

		sfb->clk_enabled = true;
	}
	mutex_unlock(&sfb->clock_lock);
}

/* Called with clock_lock */
void s3c_fb_disable_display_clock(struct s3c_fb *sfb)
{
	if(sfb->clk_down && sfb->psr_info.inStateOfPSR != false) {
		clk_down_request=true;
	}

	if (sfb->clk_gating && sfb->clk_enabled) {
		clk_gating_request=true;
#if 1 // goes to s5p_mipi_dsi.c framedone interrupt handler
		if (!sfb->variant.has_clksel)
...
#ifdef CONFIG_FB_VIDEO_PSR
static void s3c_fb_psr_enable(struct s3c_fb *sfb)
{
	struct s3c_fb_platdata *pd = sfb->pdata;
	struct device *dsim_device = (struct device*)pd->dsim1_device;
	struct platform_device *pdev = to_platform_device(dsim_device);
	struct mipi_dsim_device *dsim = platform_get_drvdata(pdev);
	u32 vidcon0, fimd_count=0;
	unsigned int ret;

	// already checked currently in video through mode
	// already checked PSR Mode threshold
	// get in VIDEO PSR
	// always enter after TE Falling event
	if (!dsim->enabled)
		return;

	sfb->psr_info.inStateOfChangeToPSR = true;
	// turning on transition state
	// first interrupt, 1 step V2CRM=1, DM=0 setting
	dsim->dsim_lcd_drv->gopsr(dsim, true);
	usleep_range(17*1000, 17*1000);
	// Skip one frame for Capturing and reserve per-frame off
	ret = s3c_fb_wait_for_te(sfb, 17);
	if (ret) {
		//dev_dbg(sfb->dev, "timeout\n");
		// Canceling case maybe suspending
		if(sfb->psr_info.inStateOfChangeToPSR == false)
			return;
	}
	// second interrupt, 2 step per-frame off, LCD Module is in the capture mode
	...
	if (vidcon0 & VIDCON0_ENVID) {
		...
		do {
			...
			if (!(readl(sfb->regs + VIDCON0) & VIDCON0_ENVID_F))
				break;
			usleep_range(1000, 1000);
		} while (1);
	} else
		dev_err(sfb->dev, "FIMD is not running @ %s\n", __func__);
	// enable SW TE interrupt // always on. just masking HW TE
	// next TE interrupt
	// Video Block Off
	// Disable SW TE // Always on just masking HW TE
	// ensure no activate HW Trigger
	s3c_fb_psr_trigger_set(sfb, 0);
	mutex_lock(&sfb->psr_info.lock);
	sfb->psr_info.inStateOfChangeToPSR = false;
	sfb->psr_info.inStateOfPSR = true;
	mutex_unlock(&sfb->psr_info.lock);
...
static void s3c_fb_psr_disable(struct s3c_fb *sfb)
{
	...
	struct platform_device *pdev = to_platform_device(dsim_device);
	struct mipi_dsim_device *dsim = platform_get_drvdata(pdev);
	u32 vidcon0, vidtcon3;

	if (!dsim->enabled)
		return;
	// get in VIDEO THROUGH
	//pr_info("last mif %d, int %d\n", last_mif_qos, last_int_qos);
	pm_qos_update_request(&exynos5_mif_qos, last_mif_qos);
	pm_qos_update_request(&exynos5_int_qos, last_int_qos);
	// check if currently in video PSR mode
	if (!sfb->psr_info.inStateOfPSR) return;
	// check if currently in transition to PSR
#if 0
	if (sfb->psr_info.inStateOfChangeToPSR)
		//wait for TE and recheck PSR
		while (!sfb->psr_info.inStateOfChangeToPSR) {
			// just skipping one frame
			ret = s3c_fb_wait_for_te(sfb, 17);
		}
	// enable SW TE IRQ // always on just masking HW TE
	// get in TE Falling Edge
	ret = s3c_fb_wait_for_te(sfb, 50);
	if (ret) {
		dev_err(sfb->dev, "wait timeout : %s\n", __func__);
	}
#endif 
	/*go entering into video mode without waiting frames.*/
	// V2CRM=0, DM=1 setting
	dsim->dsim_lcd_drv->gopsr(dsim, false);

	vidtcon3 = readl(sfb->regs + VIDTCON3);
	writel(VSYNCEN | vidtcon3, sfb->regs + VIDTCON3);
	//gpio_set_value(MEIZU_VOLDOWN_IRQ, 1);	
	// PSR on <- TRIGCON[0]=0, [3]=1,[4]=1
	s3c_fb_psr_trigger_set(sfb, 1);
	// turn on FIMD Clock
	vidcon0 = readl(sfb->regs + VIDCON0);
	vidcon0 |= VIDCON0_ENVID | VIDCON0_ENVID_F;// | VIDCON2_WB_ENABLE;
	writel(vidcon0, sfb->regs + VIDCON0);

	mutex_lock(&sfb->psr_info.lock);
	sfb->psr_info.inStateOfPSR=false;
	mutex_unlock(&sfb->psr_info.lock);
	// just leave. automatically on at next TE rising
	// on next TE disable SW TE interrupt // always on just masking HW TE
}
#endif
...
#if defined(CONFIG_FB_VIDEO_PSR)
	    s3c_fb_enable_display_clock(sfb);
	    clk_down_request=false;
	    mutex_lock(&sfb->psr_info.lock);
	    if (sfb->psr_info.inStateOfPSR) {
		    mutex_unlock(&sfb->psr_info.lock);
		    sfb->psr_info.goPSR=false;
		    s3c_fb_psr_disable(sfb);
	    } else {
		    mutex_unlock(&sfb->psr_info.lock);
		    if(sfb->psr_info.inStateOfChangeToPSR) {
			    while (!sfb->psr_info.inStateOfPSR || sfb->psr_info.inStateOfChangeToPSR) {
				    usleep_range(17*1000, 17*1000);
			    }
			    // We hurry going back to video mode
			    sfb->psr_info.goPSR = false;
			    s3c_fb_psr_disable(sfb);
		    }
	    }
...
static irqreturn_t s3c_fb_te_irq(int irq, void *dev_id)
{
...
#ifdef CONFIG_FB_VIDEO_PSR
	static u32 frame_noupdate_count = 0;	
#endif

#ifdef INTERRUPT_LATENCY_CHECK
	u64 interrupt_latency_nano;
	static u32 new_gpio_base=0;
	static u32 new_GPX1_base=0;

	interrupt_latency_nano = local_clock() - interrupt_timestamp[smp_processor_id()];
#endif
	ktime_t timestamp = ktime_get();

	spin_lock(&sfb->slock);
	// gpio_set_value(MEIZU_VOLUP_IRQ, 0);
#if defined(CONFIG_FB_VIDEO_PSR)
	sfb->te_info.timestamp = timestamp;
	wake_up_interruptible_all(&sfb->te_info.wait);
#else
	...
	te_count++;
#ifdef CONFIG_FB_VIDEO_PSR
	if (sfb->s3c_fb_update_request == 0 && !sfb->psr_info.inStateOfPSR) 
		frame_noupdate_count++;
	else
		frame_noupdate_count = 0;
	if (frame_noupdate_count > PSR_THRESHOLD) {
		// Trigger PSR
		if (!sfb->psr_info.inStateOfPSR) { 
			sfb->psr_info.goPSR=true;
			sfb->psr_info.frame_noupdate_count=frame_noupdate_count;
			wake_up_interruptible_all(&sfb->psr_info.wait);
			frame_noupdate_count = 0;
		}
		//gpio_set_value(MEIZU_VOLUP_IRQ, 1);
		// TODO Disable TE -> NO SW Interference.
	}
	if (sfb->s3c_fb_update_request > 0) {
		frame_noupdate_count = 0;
		if(sfb->psr_info.inStateOfPSR){
			sfb->psr_info.goPSR=false;
			sfb->psr_info.frame_noupdate_count=frame_noupdate_count;
			wake_up_interruptible(&sfb->psr_info.wait);
		}
	}
#endif
#ifdef CONFIG_S3C_FPS_SELFADJUST
	//Applying will be done after TE VSYNC
	if (sfb->s3c_fb_update_request == 0)
		frame_noupdate_count++;
	// Descrease MIF QOS
	if (frame_noupdate_count > FRAMEDOWN_THRESHOLD) {
		//Not working in ISR
		//		pm_qos_update_request(&exynos5_mif_qos, 0);
		//		pm_qos_update_request(&exynos5_int_qos, 0);
		//       last_mif_qos = 0;
...
#ifdef CONFIG_S3C_FPS_ADJUSTING
    // We assume Request come in Vertical Front Porch Interrupt
	switch(sfb->target_fps) {
		case FPS_40HZ:
			//s3c_fb_fps_adjust_to_40_in_vfp(sfb);
			gpio_set_value(MEIZU_VOLUP_IRQ, 0);
			data = readl(sfb->regs + VIDCON0);
			clkdiv = (data & (0xFF<<6))>>6; // This value is logically +1
			data &= ~(VIDCON0_CLKVAL_F_MASK | VIDCON0_CLKDIR);
			data |= VIDCON0_CLKVALUP;
			data |= VIDCON0_CLKVAL_F(clkdiv + 1);// | VIDCON0_CLKDIR; // increase divider
			writel(data, sfb->regs + VIDCON0);
			//printk("new VIDCON0=%x\n", data);

			//  s3c_fb_enable_vsync_irq(sfb);
			sfb->target_fps = 0;
			sfb->current_fps = FPS_40HZ;
			//printk("User adjusted frame rate to 40Hz\n");
			gpio_set_value(MEIZU_VOLUP_IRQ, 1);
			break;
		case FPS_60HZ:
			//  s3c_fb_fps_adjust_to_60_in_vfp(sfb);
			gpio_set_value(MEIZU_VOLUP_IRQ, 0);
			data = readl(sfb->regs + VIDCON0);
			clkdiv = (data & (0xFF<<6))>>6; // This value is logically +1
			data &= ~(VIDCON0_CLKVAL_F_MASK | VIDCON0_CLKDIR);
			if (clkdiv >= 1){
				...
				data &= ~VIDCON0_CLKDIR; // never run
			writel(data, sfb->regs + VIDCON0);
			//printk("new VIDCON0=%x\n", data);

			//  s3c_fb_enable_vsync_irq(sfb);
			sfb->target_fps = 0;
			sfb->current_fps = FPS_60HZ;
			//printk("User adjusted frame rate to 60Hz\n");
			gpio_set_value(MEIZU_VOLUP_IRQ, 1);
			break;
		default:
			break;
...
#endif
#ifdef CONFIG_S3C_FPS_SELFADJUST
	//Applying will be done in next VSYNC
	if (sfb->s3c_fb_update_request == 0)
		frame_noupdate_count++;
	// this is processed in VSYNC
	if ((frame_noupdate_count > FRAMEDOWN_THRESHOLD) && (sfb->current_fps == FPS_60HZ)) {
		sfb->target_fps=FPS_40HZ;
		frame_noupdate_count = 0;
	}
	if (sfb->s3c_fb_update_request > 0) {
		if(sfb->current_fps == FPS_40HZ)
		{
			sfb->target_fps=FPS_60HZ;
		}
		frame_noupdate_count = 0;
...
#ifndef CONFIG_S3C_FPS_ADJUSTING
...
static int s3c_fb_wait_for_te(struct s3c_fb *sfb, u32 timeout)
{
	...
	timestamp = sfb->te_info.timestamp;
	//s3c_fb_activate_vsync(sfb);
	if (timeout) {
		ret = wait_event_interruptible_timeout(sfb->te_info.wait,
				!ktime_equal(timestamp,	sfb->te_info.timestamp),
				msecs_to_jiffies(timeout));
	} else {
		ret = wait_event_interruptible(sfb->te_info.wait,
				!ktime_equal(timestamp,	sfb->te_info.timestamp));
	}
	//s3c_fb_deactivate_vsync(sfb);
		...
		struct s3c_fb_win_config_data *win_data)
{
	...
	for (i = 0; i < sfb->variant.nr_windows && !ret; i++) {
...
#if defined(CONFIG_FB_LCD_WQXGA) && defined(CONFIG_FB_EXYNOS_FIMD_SYSMMU_DISABLE)
		...
		if (enabled && config->state == S3C_FB_WIN_STATE_BUFFER) {
			...
			dev_dbg(sfb->dev, "bpp %d, w %d, h %d, fps %d\n", win->fbinfo->var.bits_per_pixel,
			config->w, config->h, win->fps);
		}
	}

	regs->bandwidth = bw;

	dev_dbg(sfb->dev, "Total BW = %d Mbits, Max = %d Mbits\n",
...
static	int mif_qos_tbl[]={100000,200000,400000,800000};
static	int int_qos_tbl[]={50000,100000,160000,200000,267000,400000,700000,800000};
static	int mif_tbl_size = ARRAY_SIZE(mif_qos_tbl);
static	int int_tbl_size = ARRAY_SIZE(int_qos_tbl);

static void s3c_fb_update_pm_qos(struct s3c_fb *sfb, struct s3c_reg_data *regs)
{

	int chosen_mif, chosen_int;
	bandwidth = regs->bandwidth;
	// Required FIMD Operating Clock = bandwidth/32
	// Required DMA Bus Clock = bandwidth*1.4
	for (chosen_mif = 0; chosen_mif < mif_tbl_size; chosen_mif++) {
		if (bandwidth/1000/2 < mif_qos_tbl[chosen_mif]) {
			break;
		}
	}
	for (chosen_int = 0; chosen_int < int_tbl_size; chosen_int++) {
		if (bandwidth/1000/4 < int_qos_tbl[chosen_int]){
			break;
		}
	}
	if (chosen_mif >= mif_tbl_size)
		chosen_mif = mif_tbl_size - 1;
	if (chosen_int >= int_tbl_size)
	       	chosen_int = int_tbl_size - 1;
	//pr_info("%s mif %d, int %d\n", __func__, chosen_mif, chosen_int);
#if defined(CONFIG_FIMD_USE_BUS_DEVFREQ)
	pm_qos_update_request(&exynos5_mif_qos, mif_qos_tbl[chosen_mif]);
	last_mif_qos = mif_qos_tbl[chosen_mif];
	pm_qos_update_request(&exynos5_int_qos, int_qos_tbl[chosen_int]);
	last_int_qos = int_qos_tbl[chosen_int];
#endif
}
	...
	do {
		...
		for (i = 0; i < sfb->variant.nr_windows; i++) {
			...
			if (unlikely(new_start != shadow_start)) {
				pr_info("%s win %d, ret %d, count %d", __func__, i, ret, count);
				wait_for_vsync = true;
				break;
			}
		}
					 ...
					 struct s3c_fb_win *win)
{
	...
	fbi->screen_base = ion_map_kernel(sfb->fb_ion_client, handle);
	memset(fbi->screen_base, 0, size);
	...
	pr_debug("%s smem_start 0x%x, size %d\n", __func__, map_dma, size);
				      ...
				      struct s3c_fb_win **res)
{
	...
	init_waitqueue_head(&sfb->te_info.wait);
	init_waitqueue_head(&sfb->psr_info.wait);
...
static void s3c_fb_clear_win(struct s3c_fb *sfb, int win)
{
	...
	dev_dbg(sfb->dev, "%s:shadow_con:%x\n", __func__, readl(sfb->regs + SHADOWCON));	
...
static int s3c_fb_sd_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	dev_info(sfb->dev, "Get the window via local path started/stopped : %d\n",
			enable);
	return 0;
}
...
#if defined(CONFIG_FB_VIDEO_PSR)
static int s3c_fb_psr_trigger_thread(void *data)
{
	struct s3c_fb *sfb = data;
	...
	while (!kthread_should_stop()) {
		wait_event_interruptible(sfb->psr_info.wait,
				sfb->psr_info.frame_noupdate_count>PSR_THRESHOLD);
		sfb->psr_info.frame_noupdate_count = 0;
		mutex_lock(&sfb->clock_lock);
		if (sfb->clk_enabled == true) {
			if (sfb->psr_info.goPSR) {
				// we have frame processing request, cancel getting in psr
				if (!sfb->vsync_info.irq_refcount){
					s3c_fb_psr_enable(sfb);
					s3c_fb_disable_display_clock(sfb);
				}
			} else {
				pr_info("HErE???");
				s3c_fb_psr_disable(sfb);
...
#ifdef CONFIG_M6X_LOGO_UNICOM
#include "unicom.h"
#else
#include "mz_logo.h"
#endif
static void __devinit s3c_fb_copy_logo_fb(void *screen_base)
{
       
#ifdef CONFIG_M6X_LOGO_UNICOM
	memset(screen_base, 0xff, 1080*1800*4);
#endif
	memcpy(screen_base + 1080 * 4 * MZ_LOGO_UPPER_MARGIN, mz_logo,
		       MZ_LOGO_HEIGHT * 1080 * 4);
}
#else
		...
		struct dma_buf *dest_buf)
{
	...
	int ret = 0;
	size_t i;
	size_t size = 1080*1800*4;
	void * framebuf = kmalloc(PAGE_SIZE, GFP_KERNEL);
	memset(framebuf, 0xff, PAGE_SIZE);

	ret = dma_buf_begin_cpu_access(dest_buf, 0, size/*resource_size(res)*/,
			DMA_TO_DEVICE);
	if (ret < 0) {
	...
	for (i = 0; i < 1080 * 1800 * 4; i += PAGE_SIZE) {
		void *to_virt = dma_buf_kmap(dest_buf, i / PAGE_SIZE);
		memcpy(to_virt, framebuf, PAGE_SIZE);
		dma_buf_kunmap(dest_buf, i / PAGE_SIZE, to_virt);
	}
	dma_buf_end_cpu_access(dest_buf, 0, size,/*resource_size(res),*/ DMA_TO_DEVICE);
err:
	kfree(framebuf);
	return ret;
}
...
static int __devinit __maybe_unused s3c_fb_clear_fb(struct s3c_fb *sfb,
...
static int s3c_fb_debugfs_show(struct seq_file *f, void *offset)
{
	...
	seq_printf(f, "bandwidth=%d\n", bandwidth);
	seq_printf(f, "shawdow = %x g_miffreq = %d\n", readl(sfb->regs + SHADOWCON), g_miffreq);
	seq_printf(f, "sysmmu_disabled= %d\n", sfb->sysmmu_disabled);
	seq_printf(f, "Requested Last QOS : MIF: %d, INT: %d\n", last_mif_qos, last_int_qos);
	seq_printf(f, "clk_enabled:%d, clk_gating_request:%d, clk_down_request:%d\n", sfb->clk_enabled, clk_gating_request, clk_down_request);
...
#ifdef CONFIG_S3C_FPS_ADJUSTING
	u32 read_temp;
	void __iomem *addr_temp;
#endif
	struct samsung_gpio_chip *chip = samsung_gpiolib_getchip(MEIZU_LCD_TE); // GPJ is LB Group
	u32 gpio_base_lb = (u32)(chip->base)&0xfffff000;
//	struct samsung_gpio_chip *chip1 = samsung_gpiolib_getchip(MEIZU_VOLUP_IRQ); //GPX is RT Group
//	u32 gpio_base_rt = (u32)(chip1->base)&0xfffff000;
//	gpio_gic_base=gpio_base_rt;
//	dev_dbg(dev, "fill gpio_base_address to gpio_base_lb:0x%x\n", gpio_base_lb);
//	dev_dbg(dev, "fill gpio_base_address to gpio_gic_base:0x%x\n", gpio_gic_base);

	// Meizu Specific Debuggin code
#if 0
	s3c_gpio_cfgpin(MEIZU_VOLDOWN_IRQ, S3C_GPIO_OUTPUT);
	s3c_gpio_setpull(MEIZU_VOLDOWN_IRQ, GPIO_PULL_UP);
	s3c_gpio_cfgpin(MEIZU_VOLUP_IRQ, S3C_GPIO_OUTPUT);
	s3c_gpio_setpull(MEIZU_VOLUP_IRQ, GPIO_PULL_UP);
#endif

	platid = platform_get_device_id(pdev);
	fbdrv = (struct s3c_fb_driverdata *)platid->driver_data;

	if (unlikely(fbdrv->variant.nr_windows > S3C_FB_MAX_WIN)) {
	...
	spin_lock_init(&sfb->fps_lock);

	mutex_init(&sfb->output_lock);
	mutex_init(&sfb->clock_lock);
	...
	// Use internal FIMD Video Frame interrupt as VSYNC 
	res = platform_get_resource(pdev, IORESOURCE_IRQ, 0);
	if (!res) {
		dev_err(dev, "failed to acquire irq resource 0\n");
		ret = -ENOENT;
		goto err_lcd_clk;
	}
...
#if 0
	printk("end of confiugre_lcd trigcon:%#x, vidcon0:%#x, vidcon1:%#x, wincon0:%#x, vidintcon0:%#x, vidtcon3:%#x, i80ifcona(0):%#x, i80ifcona(1):%#x\n",
		readl(sfb->regs + trigcon),
		readl(sfb->regs + vidcon0),
		readl(sfb->regs + vidcon1),
		readl(sfb->regs + 0x20),//wincon0),
		readl(sfb->regs + vidintcon0),
		readl(sfb->regs + vidtcon3),
		readl(sfb->regs + i80ifcona(0)),
		readl(sfb->regs + i80ifcona(1))		
    );
#endif
	...
	g_exynos5_mif_qos=&exynos5_mif_qos;
	g_exynos5_int_qos=&exynos5_int_qos;
#endif
...
#ifndef CONFIG_MACH_M6X
/*
* We do this in gsc driver instead since s3c-fb probes early now.
*/
#ifdef CONFIG_FB_EXYNOS_FIMD_MC
	...
	*/
...
#ifdef CONFIG_S3C_FPS_ADJUSTING
	ret = device_create_file(sfb->dev, &dev_attr_framerate);
	if (ret) {
		dev_err(sfb->dev, "failed to create framerate file\n");
		goto err_create_file;
	}
	sfb->current_fps = FPS_60HZ;
#endif
...
#ifdef CONFIG_S3C_FPS_ADJUSTING
	/* Set TestPoint 111 as output of VSYNC */ // SMDK
#if 0 
	addr_temp = ioremap(0x14000000, 0x4); //GPJ0
	read_temp = readl(addr_temp);
	read_temp |= 1<<13;
	writel(read_temp, addr_temp);
	iounmap(addr_temp);
#endif

	/* CLK_DIV_DISP10 physical address = 0x1002052C */
	sfb->clk_div_disp10 = ioremap(0x1002052C, 0x4);
#endif

#ifdef CONFIG_ION_EXYNOS
#if !defined(CONFIG_FB_EXYNOS_FIMD_SYSMMU_DISABLE)
#ifdef CONFIG_MACH_M6X
	s3c_fb_copy_logo_fb(sfb->windows[default_win]->fbinfo->screen_base);
#else
...
#if defined(CONFIG_FB_VIDEO_PSR)  // Need TE Signal
	sfb->irq_te_no = gpio_to_irq(MEIZU_LCD_TE); 
	ret = request_irq(sfb->irq_te_no, s3c_fb_te_irq,
				IRQF_TRIGGER_RISING, "s3c_fb_te", sfb);
#endif
	if(ret) {
		dev_err(dev, "te sw irq request failed\n");
		goto err_lcd_clk;
	}
	// set first shot for clean screen.
	//__s3c_fb_te_oneshot(sfb);
	sfb->s3c_fb_update_request = 1;
	sfb->te_irq_disabled = false;

	// Set Max Prio Group
	writel(2, gpio_base_lb+0xB10);
	// Set Max Prio EINT_NUM
	writel(7, gpio_base_lb+0xB18);
#if 0
	printk("Set Max Prio Group = 2, Max EIN_NUM=7\n");
	printk("EXT_INT_GRPFIXPRI : 0x%0x = 0x%0x\n", gpio_base_lb + 0xB10, readl((gpio_base_lb)+0xB10));
	printk("EXT_INT24_FIXPRI : 0x%0x = 0x%0x\n", gpio_base_lb + 0xB18, readl((gpio_base_lb)+0xB18));
	//    writel(0xffffffff, sfb->regs + QOS_LUT07_00);
	//    writel(0xffffffff, sfb->regs + QOS_LUT15_08);
	//    writel(0x1, sfb->regs + QOS_CTRL);
#endif

	s3c_fb_set_par(sfb->windows[default_win]->fbinfo);
#if defined(CONFIG_FB_VIDEO_PSR)
	mutex_init(&sfb->psr_info.lock);
	sfb->psr_info.thread = kthread_run(s3c_fb_psr_trigger_thread, sfb, "psr_trigger");
	if (sfb->psr_info.thread == ERR_PTR(-ENOMEM)) {
		dev_err(sfb->dev, "failed to run te thread\n");
		sfb->psr_info.thread = NULL;
	}
#endif
	...
	sfb->clk_enabled = true;
	clk_enabled = &sfb->clk_enabled;
#if defined(CONFIG_FB_VIDEO_PSR)
	sfb->clk_gating = false;
	sfb->clk_down = false;//true;
#endif
	g_sfb = sfb;
	//s3c_fb_dump_registers(sfb);
	return 0;
...
void s3c_fb_enable_global(void)
{
	s3c_fb_enable(g_sfb);	
}
EXPORT_SYMBOL_GPL(s3c_fb_enable_global);
static int s3c_fb_disable(struct s3c_fb *sfb)
{
	u32 vidcon0;
	int ret = 0;
	unsigned int fimd_count = 0;
	...
	if (sfb->clk_enabled) {
		...
		if (ret == -EBUSY) {
			...
			vidcon0 = readl(sfb->regs + VIDCON0);
			vidcon0 &= ~(VIDCON0_ENVID_F | VIDCON0_ENVID);
			writel(vidcon0, sfb->regs + VIDCON0);
		}
	}
	mutex_unlock(&sfb->clock_lock);
...
#endif
err:
	// do not enter into psr mode if err occurs.
	sfb->vsync_info.irq_refcount = 0;
	mutex_unlock(&sfb->output_lock);
	return ret;
}
void s3c_fb_disable_global(void)
{
	s3c_fb_disable(g_sfb);	
}
EXPORT_SYMBOL_GPL(s3c_fb_disable_global);
...
static int s3c_fb_enable(struct s3c_fb *sfb)
{
	...
	default_win = sfb->pdata->default_win;
	s3c_fb_configure_lcd(sfb, &pd->win[default_win]->win_mode);
	pr_debug("end of confiugre_lcd TRIGCON:%#x, VIDCON0:%#x, VIDCON1:%#x, WINCON0:%#x, VIDINTCON0:%#x, VIDTCON3:%#x, I80IFCONA(0):%#x, I80IFCONA(1):%#x\n",
		readl(sfb->regs + TRIGCON),
		readl(sfb->regs + VIDCON0),
		readl(sfb->regs + VIDCON1),
		readl(sfb->regs + 0x20),//WINCON0),
	...
	sfb->output_on = true;
	dev_dbg(sfb->dev,"%s: All enabling==Resume squence is done\n", __func__);
err:
	mutex_unlock(&sfb->output_lock);
	return ret;
}
...
static int s3c_fb_resume(struct device *dev)
{
	...
	sfb->output_on = true;
    dev_info(sfb->dev,"%s: All Resume squence is done\n", __func__);
...
static int s3c_fb_runtime_suspend(struct device *dev)
{
	...
	if (!s3c_fb_clk_validation(sfb))
		...
		dev_dbg(sfb->dev, "Clock is enabled, need to off, clk_enabled=%d\n", sfb->clk_enabled);	
...
#if defined(CONFIG_FB_VIDEO_PSR)
	mutex_lock(&sfb->psr_info.lock);
	if (sfb->psr_info.inStateOfChangeToPSR==true){
		sfb->psr_info.inStateOfChangeToPSR=false;
	}
	if (sfb->psr_info.inStateOfPSR==true){
		sfb->psr_info.inStateOfPSR=false;
	}
	mutex_unlock(&sfb->psr_info.lock);
#endif
...
#if !defined(CONFIG_ARM_EXYNOS5410_BUS_DEVFREQ)
	if (!sfb->fb_mif_handle) {
		sfb->fb_mif_handle = exynos5_bus_mif_min(400000);// 800, 667, 533, 400, 267, 200, 160, 100
		if (!sfb->fb_mif_handle)
			dev_err(sfb->dev, "failed to request min_freq for mif\n");
	}
	dev_err(sfb->dev, "No need\n");

	if (!sfb->fb_int_handle) {
		sfb->fb_int_handle = exynos5_bus_int_min(160000); // 800, 700, 400, 267, 200, 160, 100, 50
		if (!sfb->fb_int_handle)
			dev_err(sfb->dev, "failed to request min_freq for int\n");
	}
#endif
...
first_skip:
	...
	dev_dbg(sfb->dev, "Already Enabled????:%d\n", sfb->clk_enabled);
	...
	if(sfb->clk_down)
	{
		//recover FIMD Source Clock
	}
#endif

	writel(pd->vidcon1, sfb->regs + VIDCON1);
	dev_dbg(sfb->dev,"%s: All run-Resume squence is done\n", __func__);
