...
static DECLARE_DEFERRED_WORK(bt_perf_work, bt_qos_func);
#endif
...
static int s3c24xx_serial_resume_noirq(struct device *dev)
{
	...
	//May some problem??????? 20130327 --> by lt
	if (!console_suspend_enabled && port && uart_console(port)) {
		printk("%s , port %d\n",__func__, port->line);
		clk_enable(ourport->clk);
