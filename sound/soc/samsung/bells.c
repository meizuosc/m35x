...
static struct snd_soc_dai_link bells_dai_wm5102[] = {
	{
		.name = "CPU",
		.stream_name = "CPU Playback/Record",
...
static struct snd_soc_card bells_card = {
	.name = "Bells WM5102",
	.owner = THIS_MODULE,
...
static __devinit int bells_probe(struct platform_device *pdev)
{
	...
	bells_card.dev = &pdev->dev;
	ret = snd_soc_register_card(&bells_card);
	if (ret) {
		dev_err(&pdev->dev,
...
{
	snd_soc_unregister_card(&bells_card);
