...
#define BIT_JACK_NO_DEVICE      (0<<0)
#define BIT_JACK_HEADSET        (1<<0)
#define BIT_JACK_HEADSET_NO_MIC (1<<1)
...
void snd_soc_jack_report(struct snd_soc_jack *jack, int status, int mask)
{
    ...
    if (mask & SND_JACK_HEADSET) {
        if (status & SND_JACK_MICROPHONE)
            switch_set_state(&android_switch, BIT_JACK_HEADSET);
        else if (status & SND_JACK_HEADPHONE)
            switch_set_state(&android_switch, BIT_JACK_HEADSET_NO_MIC);
        else
            switch_set_state(&android_switch, BIT_JACK_NO_DEVICE);
    }
#endif
