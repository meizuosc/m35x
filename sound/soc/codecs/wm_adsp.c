...
static int wm_adsp_setup_algs(struct wm_adsp *dsp)
{
	...
	case WMFW_ADSP2:
		...
		tmp_alg = (char *)(&adsp2_id);
		for (i = 0; i < sizeof(adsp2_id) / 2; i++) {
			ret = regmap_read(regmap, mem->base + i, &val1);
			if (ret != 0) {
				adsp_err(dsp, "Failed to read firmware version \n");
				goto out;
			}
			*(tmp_alg++) = ((val1 & 0xff00) >> 8);
			*(tmp_alg++) = val1 & 0xff;
		}
	...
	for (i = 0; i < (term - pos); i++) {
		ret = regmap_read(regmap, mem->base + pos + i, &val1);
		if (ret != 0) {
			adsp_err(dsp, "Failed to read firmware version \n");
			goto out;
		}
