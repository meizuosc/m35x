		    ...
		    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_write(codec, WM8993_LEFT_OUTPUT_VOLUME, snd_soc_read(codec, WM8993_LEFT_OUTPUT_VOLUME));
		snd_soc_write(codec, WM8993_RIGHT_OUTPUT_VOLUME, snd_soc_read(codec, WM8993_RIGHT_OUTPUT_VOLUME));
#endif
		break;
...
static int mic_supply_event(struct snd_soc_dapm_widget *w,
                           struct snd_kcontrol *kcontrol, int event)
{
    struct snd_soc_codec *codec = w->codec;
    ...
    switch (event) {
    case SND_SOC_DAPM_PRE_PMU:
        if (!strcmp(w->name,"MICBIAS1")) {
            snd_soc_update_bits(codec, WM8993_POWER_MANAGEMENT_1,
                    WM8993_MICB1_ENA_MASK, WM8993_MICB1_ENA);
        }
        else if (!strcmp(w->name,"MICBIAS2")) {
            snd_soc_update_bits(codec, WM8993_POWER_MANAGEMENT_1,
                    WM8993_MICB2_ENA_MASK, WM8993_MICB2_ENA);
        }
        else
            dev_err(codec->dev, "Unknown widget: %s\n",
            w->name);
        break;
    ...
    case SND_SOC_DAPM_PRE_PMD:
        if (!strcmp(w->name, "MICBIAS1"))
            snd_soc_update_bits(codec, WM8993_POWER_MANAGEMENT_1,
                    WM8993_MICB1_ENA_MASK, 0);
        else if (!strcmp(w->name,"MICBIAS2"))
            snd_soc_update_bits(codec, WM8993_POWER_MANAGEMENT_1,
                    WM8993_MICB2_ENA_MASK, 0);
        else
            dev_err(codec->dev, "Unknown widget: %s\n",
            w->name);
        break;
...
#ifndef CONFIG_SND_SOC_SAMSUNG_TF4_WM1811
SND_SOC_DAPM_SUPPLY("MICBIAS2", WM8993_POWER_MANAGEMENT_1, 5, 0, NULL, 0),
SND_SOC_DAPM_SUPPLY("MICBIAS1", WM8993_POWER_MANAGEMENT_1, 4, 0, NULL, 0),
#else
SND_SOC_DAPM_SUPPLY("MICBIAS1", SND_SOC_NOPM, 0, 0, mic_supply_event,
            SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_PRE_PMD),
SND_SOC_DAPM_SUPPLY("MICBIAS2", SND_SOC_NOPM, 0, 0, mic_supply_event,
            SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_PRE_PMD),
#endif
...
#else
    { "MICBIAS1", NULL, "IN1LP" },
    { "IN1L PGA", "IN1LP Switch", "MICBIAS1" },
#endif
...
#else
    { "MICBIAS2", NULL, "IN1RP" },
    { "IN1R PGA", "IN1RP Switch", "MICBIAS2" },
#endif
