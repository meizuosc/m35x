...
const char *arizona_rate_text[ARIZONA_RATE_ENUM_SIZE] = {
	"SYNCCLK rate", "8kHz", "48kHz", "ASYNC Rate1", "ASYNC Rate 2",
};
...
//				   ARIZONA_IN_VU, val);
		regmap_read(priv->arizona->regmap,
			    ARIZONA_ADC_DIGITAL_VOLUME_1L + (i * 4),
			    &tmp);
		tmp &= ~ARIZONA_IN_VU;
		tmp |= val;
		regmap_write(priv->arizona->regmap,
			    ARIZONA_ADC_DIGITAL_VOLUME_1L + (i * 4),
			    tmp);
	}
}
		  ...
		  int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		/* ARIZONA_IN_VU bit set failed, that lead to single pass of mobile telephone. write three times to fix it */
		printk("%s: retry %d\n", __func__, retry);
		do {
			mdelay(1);
...
static void arizona_out_set_vu(struct snd_soc_codec *codec, int ena)
{
	struct arizona_priv *priv = snd_soc_codec_get_drvdata(codec);
	unsigned int val;
	int i;
	...
	for (i = 0; i < 3; i++)
		regmap_update_bits(priv->arizona->regmap,
				   ARIZONA_DAC_DIGITAL_VOLUME_1L + (i * 4),
				   ARIZONA_OUT_VU, val);
}
...
#if 0
static int arizona_prepare(struct snd_pcm_substream *substream,
			   struct snd_soc_dai *dai)
{
	// struct snd_soc_codec *codec = dai->codec;
			     ...
			     struct snd_soc_dai *dai)
{
	...
	if (!strcmp(dai->name, "wm5102-aif3") || !strcmp(dai->name, "wm5102-aif2")) {
		return 0;
	}
